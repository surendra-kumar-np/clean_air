...
// ticket milestone start
    public function addMilestonesForTickets($data, $clientRequest = false)
    {
    ...
    $data['reminderone_date']      = $data['reminderone_date'];
    $data['remindertwo_date']      = $data['remindertwo_date'];
    $data['dateadded']             = date('Y-m-d H:i:s');
        ...
        if ($insert_id) {
         ...
         handle_tags_save($tags, $insert_id, $data['rel_type']);
...
// ticket milestones end
    ...
    public function assign_task_assignees($projectId,$staffId)
    {
        $this->db->where('taskid', $projectId);
        $taskAssignedData = $this->db->get(db_prefix() . 'task_assigned')->row();
        $status = 1;
        ...
        if (!$taskAssignedData) {
            ...
            // $this->projects_model->log_activity($projectId, 'not_ticket_reassigned_to_you', get_staff_full_name(get_staff_user_id()),'not_ticket_reassigned_to_you '.$staffId);
        }

        return true;
    }

    public function update_task_status($projectId){
        $this->db->where('rel_id', $projectId);
        $taskData = $this->db->get(db_prefix() . 'tasks')->result_array();
        ...
        if ($affectedRows > 0) {
            // $this->load->model('projects_model');
            // $this->projects_model->log_activity($projectId, 'project_activity_updated_milestone', get_staff_full_name(get_staff_user_id()),'Updated status 2');
            return true;
        }else{
            return false;
        }
    ...
    public function update_reopen_task_status($projectId){
        $this->db->where('rel_id', $projectId);
        $taskData = $this->db->get(db_prefix() . 'tasks')->result_array();
        ...
        foreach($taskData as $key => $data){
            ...
            if(in_array($key,[0,1])){
                ...
                $this->db->update(db_prefix() . 'tasks', ['status'=>0]);
                if ($this->db->affected_rows() > 0) {
                    $affectedRows++;
                }
            }
        ...
        if ($affectedRows > 0) {
            // $this->load->model('projects_model');
            // $this->projects_model->log_activity($projectId, 'reset_milestone', get_staff_full_name(get_staff_user_id()),'Milestone Reset');
            return true;
        }else{
            return false;
        }
    ...
    public function updateTaskStatus($projectId,$data){
        $this->db->where('taskid',$projectId);
        $this->db->update(db_prefix() . 'task_assigned', $data);
        if ($this->db->affected_rows() > 0) {
            return true;
        }
        return false;
    ...
    public function update_reopen_tasks($projectId,$task_id,$taskDetails){
        $this->db->where('id',$task_id);
        $this->db->where('rel_id',$projectId);
        $this->db->update(db_prefix() . 'tasks', $taskDetails);
