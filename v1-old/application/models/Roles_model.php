...
class Roles_model extends App_Model
{
    ...
    /**
     * Get role id role by slug_url
     * @param  mixed $slug_url Optional slug url
     * @return mixed     array if not id passed else object
     */
    public function get_role_from_slug($slug_url = '')
    {
        if (is_string($slug_url) && !empty($slug_url)) {

            $role = $this->app_object_cache->get('role-' . $slug_url);
            ...
            $this->app_object_cache->add('role-' . $slug_url, $role);
