	...
	*/
    ...
    {		
		...
		if($type == 'new') {
            ...
            $this->db->select('p.id as projectId, pm.color, "#2c77ee" as markerColor');
		...
		if($type == 'escalated') {
            ...
            $this->db->select('p.id as projectId, pm.color, "#f43653" as markerColor');
			
			$this->db->where('( (p.status IN (2, 4, 6) AND p.action_date < CURDATE() AND p.frozen = 0) OR (p.status = 1 AND p.action_date < CURDATE() AND p.frozen = 0) )');
		}
		
		//closed	#2bb47a
		if($type == 'closed') {
			
            $this->db->select('p.id as projectId, pm.color, "#2bb47a" as markerColor');
		...
		if($type == 'wip') {
            ...
            $this->db->select('p.id as projectId, pm.color, "#f5b500" as markerColor');
		...
		if($type == 'rejected') {
            ...
            $this->db->select('p.id as projectId, pm.color, "#7e007f" as markerColor');
		...
		if($type == 'unassigned') {
            ...
            $this->db->select('p.id as projectId, pm.color, "#5603b5" as markerColor');
		...
		if($type == 'frozen') {
            ...
            $this->db->select('p.id as projectId, pm.color, "#c96118" as markerColor');
