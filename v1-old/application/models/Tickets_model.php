    ...
    public function update_milestone($data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if (isset($task)) {
                ...
                $this->db->update("projects", ["status" => 4]);
                log_activity('Project status changed [ID:' . $data['project_id'] . ', status: 4]');
            }

            // Update Action Date
            $this->update_action_date($data['project_id']);

            $this->db->insert("project_notes", ["project_id" => $data['project_id'], "task_id" => $data['task_id'], "content" => $data["comment"], "staff_id" => $data['staff_id'], "created_at" => date('Y-m-d H:i:s'), "status" => $data["pStatus"]]);
            $this->load->model("projects_model");
