...
class Projects extends AdminController
{
    ...
    public function get_project_activity()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $data['activities'] = $this->projects_model->get_activity($id);
                ...
                foreach ($data["activities"] as $key => $activity) {
                    ...
                    if ($activity["contact_id"] != 0) {
                        if($activity["description_key"] == 'ticket_created_cc'){
                            $staff = $this->projects_model->get_project_details($id);
                            if (!empty($staff)) {
                                $data['activities'][$key]["staff_data"] = [
                                    "name" => $staff->rname,
                                    "email" => $staff->remail,
            ...
            // pre($data);
            $this->load->view('admin/tickets/ticket_history', $data);
        }else{
    ...
    public function view($id = 0)
    {
        if (has_permission('projects', '', 'view') || $this->projects_model->is_member($id)) {
            ...
            $project->settings->available_features = unserialize($project->settings->available_features ? $project->settings->available_features : "");
            $data['statuses']                      = $this->projects_model->get_project_statuses();
