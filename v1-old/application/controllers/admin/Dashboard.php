...
//print_r($data);exit;
    ...
    public function mapview()
    {
		...
		$data['subregion'] = $this->report_model->get_subregion();
		$data['ticket'] = $this->report_model->get_ticket();
		
		//$data['action_taker'] = $this->report_model->get_action_taker($get_area);
		//$data['action_reviewer'] = $this->report_model->get_action_reviewer($get_area);
			...
			if ($userRole == "ar") {
				...
				if (count($ticket_arr) > 0) {
					...
					if (in_array(1, $ticket_arr)){
						$dashboard_map_data1 = $this->dashboard_model->get_ae_area_map_data($tableParams, 'new');
					}
					
					//wip => 2
					if (in_array(2, $ticket_arr)){
						$dashboard_map_data4 = $this->dashboard_model->get_ae_area_map_data($tableParams, 'wip');
					}
					
					//closed => 3
					if (in_array(3, $ticket_arr)){
						$dashboard_map_data3 = $this->dashboard_model->get_ae_area_map_data($tableParams, 'closed');
					}
					
					//referred (rejected) => 5
					if (in_array(5, $ticket_arr)){
						$dashboard_map_data5 = $this->dashboard_model->get_ae_area_map_data($tableParams, 'rejected');
					}
					
					//escalated (Delayed) => 7
					if (in_array(7, $ticket_arr)){
						$dashboard_map_data2 = $this->dashboard_model->get_ae_area_map_data($tableParams, 'escalated');
					}
					
					//frozen => 8
					if (in_array(8, $ticket_arr)){
						$dashboard_map_data7 = $this->dashboard_model->get_ae_area_map_data($tableParams, 'frozen');
					}
					
					//unassigned => 9
					if (in_array(9, $ticket_arr)){
						$dashboard_map_data6 = $this->dashboard_model->get_ae_area_map_data($tableParams, 'unassigned');
					}
				...
				$dashboard_map_data = array_merge($dashboard_map_data1, $dashboard_map_data2, $dashboard_map_data3, $dashboard_map_data4, $dashboard_map_data5, $dashboard_map_data6, $dashboard_map_data7);
			}
			...
			if($dashboard_map_data) {
				...
				foreach($dashboard_map_data as $map_data) {
					...
					if($arr_files_data) {
						...
						$locInfo[] = ["<div class=ticket_details data-project_id=". $map_data['projectId'] 
						." ><h4>".$ticketDetails->project_name."</h4><p>(".$ticketDetails->landmark.")</p><p style=text-align:center;>" . $img . "</p></div>"];
					}
				}
			}
