...
function project_latest_notes($projectId,$status=NULL){
    $CI = & get_instance();
    $CI->db->select('task_id,content,status,exception');
    $CI->db->where('project_id', $projectId);
    if(!empty($status)){
        $CI->db->where('status', $status);
    }
    $CI->db->order_by('id','desc');
    $project = $CI->db->get(db_prefix() . 'project_notes')->row();
    return $project;
}

function getExceptionName($exceptionId){
    $CI = & get_instance();
    $CI->db->select('name');
    $CI->db->where('id', $exceptionId);
    return $CI->db->get(db_prefix() . 'exception')->row()->name;
}
...
function is_project_reopened($project_id)
{   
    $status = 0;
    $CI = & get_instance();
    $CI->db->where('project_id', $project_id);
    $CI->db->where('status', 6);
    $project = $CI->db->get(db_prefix() . 'project_activity')->row();
    if ($project) {
        $status = $project->status;
    }
