 ...
 * @return boolean
 */
function handle_project_file_uploads($project_id,$locationdata=[])
{                       $CI = &get_instance();
    $filesIDS = [];
    $errors   = [];
    $allowed_mime_type_arr = array('image/gif', 'image/jpeg', 'image/pjpeg', 'image/png', 'image/x-png','image/PNG','image/JPEG','image/JPG');
    if (
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (in_array($_FILES['file']['type'][$i], $allowed_mime_type_arr)) {
                $locationValue = get_image_location($tmpFilePath);
                if($locationValue ==false and !empty($locationdata)){
                    $locationValue = [
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (in_array($_FILES['file']['type'][$i], $allowed_mime_type_arr)) {
                    ...
                    $tempname = $xp['name'];
                    $tempfile_ext = $xp['ext'];
                    $thumbnailname=$tempname.'_thumb'.$tempfile_ext;
                    $thumbnaillink=$tempname.'_300px'.$tempfile_ext;
                    // thumb name end
                }else{
                    $thumbnailname=$filename;
                    $thumbnaillink=$filename;
                }
                // thumbnail changes end
                // In case client side validation is bypassed
                if (!_upload_extension_allowed($filename)) {
                    continue;
                }
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    $data = [
                        ...
                        'subject'    => $thumbnailname,
                        'milestone' => 0,
                        'latitude'  => (!empty($locationValue)) ? $locationValue['latitude'] : 0,
                        'longitude'  => (!empty($locationValue)) ? $locationValue['longitude'] : 0,
                        'address'  => (!empty($locationValue)) ? $locationValue['address'] : '',
                    ...
                    if ($insert_id) {
                        if (is_image($newFilePath)) {
                            ...
                            create_img_thumb_300($path, $filename);
                        }
...
function handle_task_attachments_array($taskid, $index_name = 'attachments')
{
    ...
    if (
        ...
        for ($i = 0; $i < count($_FILES[$index_name]['name']); $i++) {
            ...
            if (in_array($_FILES[$index_name]['type'][$i], $allowed_mime_type_arr)) {
                $locationValue = get_image_location($tmpFilePath);
                // if($locationValue ==false and !empty($browsercoordinates)){
                //     $locationValue = [
...
function create_img_thumb_300($path, $filename, $width = 1920, $height = 1080)
{
    $CI = &get_instance();
