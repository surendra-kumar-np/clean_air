...
class Client_merge_fields extends App_merge_fields
{
    ...
    public function format($client_id, $contact_id = '', $password = '')
    {
        ...
        if ($contact) {
            $fields['{citizen_companyname}']		= get_option('citizen_companyname');
 ...
 */
    ...
    public function password($data, $type)
    {
        ...
        if ($type == 'forgot') {
			if($data['company'] == 'Citizen - Citizen') {
				$fields['{reset_password_url}'] = site_url('citizensauthentication/reset_password/0/' . $data['userid'] . '/' . $data['new_pass_key']);
			} else {
