...
class AdminController extends App_Controller
{
    public function __construct()
    {
        ...
        $role = $this->roles_model->get($currentUser->role);

        if(!empty($role)){
          $currentUser->role_name = $role->name;
          $currentUser->role_slug_url = $role->slug_url;
        }
        ...
        // Fetch Area Name
        if ($currentUser->area!=0) {
            $this->load->model('area_model');
            $this->load->model('staff_region_model');
            $area_detail = $this->area_model->get($currentUser->area);
            $region_detail = $this->staff_region_model->get_staff_region($currentUser->staffid);
        //    echo "<pre>";print_r($region_detail);echo "</pre>";die();
            $area_list['area'] = $area_detail->name;
            $currentUser->area_prefix = $area_detail->prefix;
            if(!empty($region_detail)){
                $area_list['region_name'] = !empty($region_detail->region_name)?$region_detail->region_name:'';
                $area_list['region_id'] = !empty($region_detail->region_id)?$region_detail->region_id:'';
                $area_list['subregion_name'] = !empty($region_detail->subregion_name)?$region_detail->subregion_name:'';
                $area_list['subregion_list'] = !empty($region_detail->sub_region)?$region_detail->sub_region:'';
            }
            $currentUser->location = $area_list;
        }
        $currentUser->ward_name = !empty($region_detail->ward_name)?$region_detail->ward_name:'';
