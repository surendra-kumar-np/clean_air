...
class Authentication extends ClientsController
{
    ...
    {
		//redirect to citizen login in case of auto logout from citizen app
		if( isset($_SERVER['HTTP_REFERER']) ){
			$arr_url = explode( '/', $_SERVER['HTTP_REFERER'] );
			$url = end( $arr_url );
			if($url == 'citizens_login') {
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run() !== false) {
				...
				if ($captcha_insert === $contain_sess_captcha) {
					...
					$citizen = $this->Authentication_model->checkforCitizen($this->input->post('email'));
					
					if($citizen === true){
						set_alert('danger', _l('Citizens Cannot Login Here'));
...
{  
    ...
    $this->form_validation->set_rules('firstname', _l('Name'), 'required|max_length[50]');
