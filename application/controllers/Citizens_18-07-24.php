...
class Citizens extends CitizensController
{
    /**
     * @since  2.3.3
     */
    ...
    public function __construct()
    {
        ...
        hooks()->do_action('after_citizens_area_init', $this);
    }
	...
	/* old method which is not in use
	...
	public function open_ticket()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('message', 'Description', 'alpha_numeric_input|max_length[255]');
            $this->form_validation->set_rules('landmark', 'Landmark', 'required|alpha_numeric_input|max_length[255]');
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if(array_key_exists("latitude",$data) and array_key_exists("longitude",$data))
                {
                   ...
                   $email=(!empty($assignedat)) ? get_staff_email($assignedat,"") : get_staff_email("",$data['area']);
					...
					$staff_full_name =  get_staff_full_name($assignedat);
					$resolve_date = $ticketdata['deadline'];
					
					//sms send to citizen
					$client_phone = get_client_phone_number_by_id($this->session->userdata('client_user_id'));
					
					if(!empty($client_phone)) {
						$complaint_title = getcatname($data['categories']);
						
						$message = $this->load->view('sms_templates/citizen_complaint_raised', ['id' => $id, 'duedate'=> date_format($duedate, "Y-m-d"), 'complaint_title' => $complaint_title, 'assigned_to' => $staff_full_name, 'resolve_date' => $resolve_date], true);
						
						$smsUrl = SMS_API_URL . '&number=' . $client_phone . '&message=' . urlencode($message) . '&templateid=1207163723953956710';
					...
					if(!empty($assignedat)){
						$send=send_mail_template('New_ticket', $email, $assignedat,1, $id, $area, $region, $subregion, $category, $data['landmark'], $ticketdata['deadline']);
					}
					else
					{
                    ...
                    set_alert('success', _l('citizen_ticket_added_successfully'));
        ...
        $this->view('citizens_open_ticket');
        $this->layout();
    }
