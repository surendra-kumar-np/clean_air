...
class Clients extends ClientsController
{
    ...
    function getsubregion2(){
        ...
        $category =$this->issue_model->get_area_issues('','','',$regionid,$organisation_id);
        if(count($rows)>0){
            echo json_encode([
                'success' => true,
            ...
            echo json_encode([
                'success' => false,
                'message' => _l('no_muncp_zone_found'),
            ]);
        }
    }
    ...
    public function getWardList(){
        $res=getWardList($this->input->post('subregionId'));
        if(count($res)>0){
            echo json_encode([
                'success' => true,
    ...
    public function file_check(){
		$e = $t = 0;
		$allowed_mime_content_type_arr = array('image/jpeg','image/pjpeg','image/jpeg','image/png', 'image/x-png', 'application/pdf');
		...
		for($i = 0; $i < count($_FILES['file']['tmp_name']); $i++) {
			...
			if(empty($_FILES['file']['tmp_name'][$i])){
				...
				if(!in_array(mime_content_type($_FILES['file']['tmp_name'][$i]), $allowed_mime_content_type_arr) ){
        ...
        if($e == 4) {
            $this->form_validation->set_message("file_check", "Corrupted file! Please select another file.");
            return false;
        }
		...
		if($t == 4){
			...
			$this->form_validation->set_message("file_check", "Image or pdf are required");
    ...
    public function open_ticket()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->session->set_flashdata('organisation_id',$this->input->post('organisation_id'));
            //$this->session->set_flashdata('department_id',$this->input->post('department_id'));
            ...
            $this->session->set_flashdata('ward',$this->input->post('ward'));

            $this->form_validation->set_rules('area', 'State', 'required');
            $this->form_validation->set_rules('region', 'City', 'required');
            $this->form_validation->set_rules('organisation_id', 'Organization', 'required');
            ...
            $this->form_validation->set_rules('message', 'Description', 'required|max_length[500]');
            ...
            if ($this->form_validation->run() !== false) {
                ...
                $ticketdata=[
                    ...
                    'project_members'=>findAtwithWard($data['area'],$data['region'],$data['subregion'],$data['ward'],$data['categories']),
                    'landmark'=>$data['landmark'],
                    'address'=>$data['address'],
                    'start_date'=>date("Y-m-d"),
                    'deadline'=>date('Y-m-d', strtotime(date("Y-m-d"). ' + '.$miledurations[0].'days')),
                    'description'=>$data['message'],
                    'status'=>(!empty(findAtwithWard($data['area'],$data['region'],$data['subregion'],$data['ward'],$data['categories']))?1:9),
                    'area_id'=>$data['area'],
                    ...
                    'is_assigned'=>(!empty(findAtwithWard($data['area'],$data['region'],$data['subregion'],$data['ward'],$data['categories']))?1:0),
                ];
                ...
                add_new_details_to_surveyor_with_ward($this->session->userdata('client_user_id'),$data['area'],$data['region'],$data['subregion'],$data['ward']);
                ...
                if($id){
                    ...
                    foreach($milestones as $milestone){
                        ...
                        $staffAdmin = get_area_admin_by_area($data['area']);
                        ...
                        $task=[
                            ...
                            'rel_id'=>$id,
                            'rel_type'=>'project',
                            'staffId'=>!empty(findAtwithWard($data['area'],$data['region'],$data['subregion'],$data['ward'],$data['categories'])) ? findAtwithWard($data['area'],$data['region'],$data['subregion'],$data['ward'],$data['categories']) : $staffAdmin
                        ];
                   ...
                   $assignedat = !empty(findAtwithWard($data['area'],$data['region'],$data['subregion'],$data['ward'],$data['categories'])) ? findAtwithWard($data['area'],$data['region'],$data['subregion'],$data['ward'],$data['categories']) : $staffAdmin;
				   $client_id = $this->session->userdata('contact_user_id');
            if($client_id) {
                $this->load->model('Authentication_model');
                $citizenUser = $this->Authentication_model->checkforCitizencheck($client_id);
                $client_phone = $this->Authentication_model->get_phone_number($client_id);
                if($citizenUser){
                    $templateId =1007172534308630873;
                $datecurrent = date('Y-m-d');
                $assignedUser = get_staff_full_name($assignedat);
                    $citizen = 1;
                    getSmshelper($client_phone,"Hello Citizen. You have raised a complaint on the Bihar Clean Air App. Ticket ID ".$id." has been generated on ".$datecurrent." date under the heading ".getcatname($data['categories']).". The ticket has been assigned to ".$assignedUser." and will be resolved by ".$actiondate." date. Team, CAD, Bihar",$templateId,$citizen);
                    // getSmshelper($client_phone,"Hello Citizen. You have raised a complaint on the Bihar Clean Air App. Ticket ID ".$id." has been generated on ".$datecurrent." date under the heading ".getcatname($data['categories']).". The ticket has been assigned to ".$assignedUser." and will be resolved by ".$actiondate." date. Team, CAD, Bihar",$templateId,$citizen);
                ...
                if(array_key_exists("latitude",$data) and array_key_exists("longitude",$data))
                {
                    ...
                    if(!empty($data['loc'])){
                        $locData = explode(',', $data['loc']);
                        $lattitudeValue = $locData[0];
                        $longitudeValue = $locData[1];
                        $locationdata=[
                            'latitude'=>$lattitudeValue,
                ...
                // phone
                   ...
                   $organization=getitemname($data['organisation_id'],'organization');
                   $department=getitemname($data['department_id'],'department');
                   $subregion=getitemname($data['subregion'],'subregion');
                   $ward=getitemname($data['ward'],'ward');
                   $category =getcatname($data['categories'],'category');
                   if(!empty($assignedat)){
                    $send=send_mail_template('New_ticket',$email,$assignedat,1,
                    $id,$area,$region,$subregion,$ward,$category,$data['landmark'],$ticketdata['deadline']);
                   }
                   else{
    ...
    public function change_language($lang = '')
    {
        ...
        //print_r($_SESSION['client_user_id'].'-------'.$_SERVER['HTTP_REFERER']);exit;
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($projectId)) {
                ...
                $evidenceData .= '<ul class="nav nav-tabs ticket-detail-tabs">
                        <li class="active print-none"><a href="#imagePopup" data-toggle="tab">Image(s) & Document(s)</a></li>';
                    ...
                    $evidenceData .= ' <ul class="row docgallery content">';
            ...
            $evidenceData .= '</div>
                                ...
                                <!-- Document -->
                                <div class="tab-pane fade" id="documentPopup" style="display:none;">
                                    <ul class="nav nav-tabs only-print">
    ...
    public function getOrgDept()
    {
        ...
        $organizationNew = $this->Organization_model->getOrganization($this->input->post('region_id'));
    ...
    public function get_organisation_kml(){
        $organisation_id =$this->input->post('organisation_id');
        $this->load->model('report_model');
        $data['subregion'] = $this->report_model->get_subregion($area = '', '', '', $organisation_id);
        $data['organization'] = $this->report_model->get_organisation_kml($organisation_id);
        $data['ward'] = $this->report_model->get_wards('','','',$organisation_id);
        echo json_encode([
            'success' => true,
            'message' => "Successfully fetched the Organization.",
            'data' => $data
        ]);
        die;
    }
    ...
    public function get_categories(){
        $organisation_id =$this->input->post('categories');
        $region_id =$this->input->post('region_id');
        $this->load->model('report_model');
        $data['organization'] = $this->report_model->get_categories($organisation_id,$region_id);
        echo json_encode([
            'success' => true,
            'message' => "Successfully fetched the Organization.",
            'data' => $data
