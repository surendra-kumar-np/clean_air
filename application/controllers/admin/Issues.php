...
class Issues extends AdminController
{
    ...
    public function validate()
    {
        ...
        if ($this->input->post('id')) {
            ...
            if (empty($region_ids)) {
                $message = _l('City selection is required', _l('issue'));
                echo json_encode([
                    'success'              => false,
                    'message'              => $message,
					'updated_csrf_token' => $this->security->get_csrf_hash(),
    ...
    public function newissue()
    {
        ...
        if (has_permission('categories', '', 'create') or has_permission('categories_manage', '', 'create')) {
			...
			$this->load->library('form_validation');
			$this->form_validation->set_rules('issue_name', 'Action item name', 'trim|max_length[150]|required');
			
			if ($this->form_validation->run() == false) {
                echo json_encode([
                    'success'              => false,
                    'message'              => 'Invalid Action item name.',
                ]);
            }
            ...
            if (!$this->input->post('id')) {
                ...
                $issuetabledata = [
                    ...
                    'organization_id' =>json_encode($data['organization_new']),
                    'is_active' => 1,
                    'staff_id' => $staffid,
                    'area_id' => ($staffroleid == 2) ? 0 : $staffareaid,
                    'parent_issue_id' => 0,
                    'region_id'  => json_encode($data['region_id']),
                ];
    ...
    public function get_area_issues_with_ward()
    {
        $response = array(
            'success' => false,
            'message' => "Something went wrong."
        ...
        if ($this->input->post()) {
            ...
            //$all_issues = [];
            $all_issues = $this->issue_model->get_area_issues($area_id, 1);
            $existing_issues = $this->issue_model->get_existing_area_issues_with_ward($ward_id, $sub_region_id, $region_id);
            //if ($existing_issues) {
                $response = [
                    'success' => true,
                    'message' => "Issues fetched successfully.",
