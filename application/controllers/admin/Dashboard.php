...
class Dashboard extends AdminController
{
    ...
    public function dashboard_widget_data()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $referred = !empty($dashboard_widget_data->referred) ? $dashboard_widget_data->referred : 0;
            $longterm = !empty($dashboard_widget_data->longterm) ? $dashboard_widget_data->longterm : 0;
            $submitforapproval_total = !empty($dashboard_widget_data->submitforapproval) ? $dashboard_widget_data->submitforapproval : 0;
            ...
            $total_activity = $new +$reopen + $longterm + $closed + $submitforapproval_total + $rejected+$referred;
            $action_items = 0;
            $upcoming_deadline = 0;
            $recently_closed = 0;
        ...
        if (!empty($data)) {
            ...
            foreach ($data[0] as $key => $value) {
                ...
                if ($key == "new") {
                    ...
                    array_push($chart_data, ["Re-opened", $value]);
...
public function getwardmappinglist(){
