...
class Report extends AdminController
{
    ...
    {
        redirect( admin_url('reportatr/index') );//redirct on new ATR report section
    ...
    public function get_region()
    {
        ...
        public function get_department(){
            ...
            $data['ward'] = $this->report_model->get_wards($area,'','','','',$department_id);
            if (count($data['ward']) > 0 ) {
                echo json_encode([
                    'success' => true,
                    'message' => "Successfully fetched the ward list.",
                    'data' => $data
                ]);
            } else {
                echo json_encode([
    ...
    public function get_subregion()
    {
        if ($this->input->post()) {
            ...
            if ($role == 5) {
                ...
                $data['organization'] = $this->atr_report_model->get_organization('',$area,$regionds);
                $data['actionTaker'] = $this->report_model->get_project_leader_organization($area,'',$regionds);
                $data['actionReviewer'] = $this->report_model->get_project_reviewer_organization($area,'',$regionds);
                 $data['ward'] = $this->report_model->get_wards($area,'','','',$regionds);
                 $data['projectsupport'] =$this->atr_report_model->get_project_support($area,'','','',$regionds);
                 $data['category'] =$this->issue_model->get_area_issues($area,'','',$regionds);
            } else {
                $data['subregion'] = $this->report_model->get_subregion($area, $regionds);
                $data['organization'] = $this->atr_report_model->get_organization('',$area,$regionds);
                // ----
            ...
            if (count($data['subregion']) > 0 || count($data['organization']) > 0 || count($data['actionTaker']) > 0 || count($data['actionReviewer']) > 0 || count($data['ward']) > 0 || count($data['projectsupport']) > 0) {
                echo json_encode([
                    'success' => true,
    ...
    public function get_wards()
    {
        if ($this->input->post()) {
            $area = $GLOBALS['current_user']->area;
            $subregionds = $this->input->post('subregion_id');
            $role = $this->input->post('role');
            if ($role == 5) {
                $data['wards'] = $this->report_model->get_wards($area = '',$subregionds);
                //----
                $data['actionTaker'] = $this->report_model->get_project_leader_organization($area,'','',$subregionds);
                $data['actionReviewer'] = $this->report_model->get_project_reviewer_organization($area,'','',$subregionds);
                 $data['projectsupport'] =$this->atr_report_model->get_project_support($area,'','','','',$subregionds);
            } else {
                $data['wards'] = $this->report_model->get_wards($area,$subregionds);
                //----
                $data['actionTaker'] = $this->report_model->get_project_leader_organization($area,'','',$subregionds);
    ...
    public function get_project_leader() {
        ...
        if ($this->input->post()) {
            $area = $GLOBALS['current_user']->area;
            $staffid ='';
            $wardids = $this->input->post('ward_id');
            $user_at  = $this->staff_model->get_userDetails($GLOBALS['current_user']->staffid);
            $regionid = (!empty($user_at->region)) ? array($user_at->region) : '';
            $subregionid = $this->input->post('subregion');
            $role = $this->input->post('role');
            if($role!=6){
                $staffid = $GLOBALS['current_user']->staffid;
            }
            if ($role == 5) {
                $data['actionTaker'] = $this->report_model->get_project_leader($area,$wardids,$staffid);
                $data['actionReviewer'] = $this->report_model->get_project_reviewer($area ,$wardids,$staffid);
                $data['projectsupport'] =$this->atr_report_model->get_project_support($area,$wardids,'',$staffid);
                $data['category'] = $this->issue_model->get_area_issues($area, $active = 0,$staffid);
            } else {
                if($role!=6){
                    // if($role==4){
                    //     $data['action_taker'] = $this->atr_report_model->get_action_taker($area, $wardids='',$staffid,$regionid,$subregionid);
                        
                    // }
                    
                    
                }else{
                    $data['action_taker'] = $this->atr_report_model->get_action_taker($area, $wardids,'',$regionid,$subregionid);
                ...
                if($role==4){
                    ...
                    $data['projectsupport'] = $this->atr_report_model->get_project_support($area,$wardids,'',$plStaffId='',$regionid='',$subregionid);
                    $dataprojectsupport = $this->atr_report_model->get_project_support($area,$wardids,'',$plStaffId='',$regionid='',$subregionid);
                    if(count($dataprojectsupport)>0){
                        foreach($dataprojectsupport as $val){
                            $data_projectsupport []= $val['staffid'];
                        }}
                        $data['action_taker'] =  $this->atr_report_model->get_action_taker($area, $wardids = '', $data_projectsupport,$regionid='',$subregionid);      
                        $data['category'] = $this->issue_model->get_area_issues($area,1,$data_projectsupport="",$regionid);//show all categories for reporting
                    ...
                    $data['projectsupport'] =$this->atr_report_model->get_project_support($area,$wardids,'',$staffid);
                    $data_projectsupport = array();
                    $dataprojectsupport = $this->atr_report_model->get_project_support($area,$wardids,'',$staffid);
                    if(count($dataprojectsupport)>0){
                        foreach($dataprojectsupport as $val){
                            $data_projectsupport []= $val['staffid'];
                        }}
                        $data['actionTaker'] = $this->report_model->get_project_leader($area,$wardids,$data_projectsupport);
                }
                
            }

            if (!empty($data['actionTaker']) > 0 || !empty($data['actionReviewer'])>0 || !empty($data['projectsupport'])>0 || !empty($data['category'])>0) {
                echo json_encode([
                    'success' => true,
                    'message' => "Successfully fetched the project leader/reviewer/projectsupport list.",
                    'data' => $data
                ]);
            } else {
                echo json_encode([
                    'success' => false,
                    'message' => "No project leaders found.",
                ]);
            }
        }
        die;
    ...
    public function get_project_support_leader_filter() {
        if ($this->input->post()) {
            $staffid = '';
            ...
            $area = $GLOBALS['current_user']->area;
                ...
                if(!empty($this->input->post('projectreviewer_id'))){
                    $projectsupportids = $this->input->post('projectreviewer_id');
                }else{
                    if($role!=6){
                        $projectsupportids = $GLOBALS['current_user']->staffid;
                    }
                    
                }
                if ($role == 5) {
                    $data['projectsupport'] =$this->atr_report_model->get_project_support($area = '','',$projectsupportids);
                } else {
                    
                        $plStaffId = array();
                        $getPLids = $this->atr_report_model->get_action_taker($area, '',$projectsupportids);
                        if(count($getPLids)>0){
                        foreach($getPLids as $val){
                            $plStaffId []= $val['staffid'];
                        }}
                       
                        $data['projectsupport'] = $this->atr_report_model->get_project_support($area,'','',$plStaffId);
                    
                        //$data['projectsupport'] =$this->atr_report_model->get_project_support($area = '','',$projectsupportids);
                    
                    $data['action_taker'] = $this->atr_report_model->get_action_taker($area,'',$projectsupportids);
            ...
            if (count($data['projectsupport']) > 0 || count($data['action_taker']) > 0 ) {
                echo json_encode([
                    'success' => true,
                    'message' => "Successfully fetched the project support list.",
                    'data' => $data
                ]);
            } else {
                echo json_encode([
    ...
    public function get_project_support_filter() {
        if ($this->input->post()) {
            $staffid = '';
            
            $role = $this->input->post('role');
            $projectsupport_id = $this->input->post('projectsupport_id');
            $ward = $this->input->post('ward');
            $subregion = $this->input->post('subregion');
            $area = $GLOBALS['current_user']->area;
            if($this->input->post('action_taker_id')){
                $action_taker_idids = $this->input->post('action_taker_id');
                if ($role == 5) {
                    $data['projectsupport'] =$this->atr_report_model->get_project_support($area = '','','',$action_taker_idids);
                } else {
                    $data['projectsupport'] =$this->atr_report_model->get_project_support($area = '','','',$action_taker_idids);
                }
            }else{
                
                if(!empty($this->input->post('projectsupport_id'))){
                    $projectsupportids = $this->input->post('projectsupport_id');
                }else{
                    if($role!=6){
                        $projectsupportids = $GLOBALS['current_user']->staffid;
                    }
                ...
                if ($role == 5) {
                    $data['projectsupport'] =$this->atr_report_model->get_project_support($area,$ward,$projectsupportids , $plid = '', $region_id = '', $subregion);
                } else {
                    $data['projectsupport'] =$this->atr_report_model->get_project_support($area,$ward, $projectsupportids,$plid = '', $region_id = '', $subregion);
                }
            }
            ...
            if (count($data['projectsupport']) > 0 ) {
                echo json_encode([
                    'success' => true,
                    'message' => "Successfully fetched the project support list.",
                    'data' => $data
    ...
    public function get_project_organization() {
        if ($this->input->post()) {
            $area = $GLOBALS['current_user']->area;
            $organizationids = $this->input->post('organization_id');
            $role = $this->input->post('role');
            
            if ($role == 5) {
                $data['actionTaker'] = $this->report_model->get_project_leader_organization($area = '',$organizationids);
                $data['actionReviewer'] = $this->report_model->get_project_reviewer_organization($area = '',$organizationids);
                $data['subregion'] = $this->report_model->get_subregion($area = '', '', '', $organizationids);
                $data['department'] = $this->Department_model->getOrgDepartment($organizationids);
                $data['ward'] = $this->report_model->get_wards('','','',$organizationids);
            } else {
                $data['actionTaker'] = $this->report_model->get_project_leader_organization($area,$organizationids);
                $data['actionReviewer'] = $this->report_model->get_project_reviewer_organization($area,$organizationids);
                $data['subregion'] = $this->report_model->get_subregion($area,'', '', $organizationids);
            ...
            if (count($data['actionTaker']) > 0 || count($data['actionReviewer']) >0 || count($data['subregion']) >0 || count($data['ward']) >0 || count($data['department']) >0 ) {
                echo json_encode([
                    'success' => true,
                    'message' => "Successfully fetched the project leader/reviewer list.",
                    'data' => $data
                ]);
            } else {
                echo json_encode([
