...
class Reportatr extends AdminController
{
    ...
    public function index()
    {
        ...
        $subregionid = (!empty($post_data['subregion'])) ? $post_data['subregion'] : '';
        ...
        if ($role_slug == 'ae-global') {
            ...
            $data['title'] = $siteTitle[$role_slug];
            $data['action_reviewer'] = $this->atr_report_model->get_action_reviewer($area, $organizationid, $regionid, $subregionid);
            if ($role != 6) {
                $data['action_taker'] = $this->atr_report_model->get_action_taker($area, $wardid = '', $staffid, $regionid, $subregionid);

            }else if($role == 6){
                $action_reviewer_setid = array();
                if (count($data['action_reviewer']) > 0) {
                    foreach ($data['action_reviewer'] as $val) {
                        $action_reviewer_setid[] = $val['staffid'];
                    }
                }
                $data['action_taker'] = $this->atr_report_model->get_action_taker($area, $wardid = '', $action_reviewer_setid, $regionid, $subregionid);
            } else {
            ...
            if ($role != 6) {
                if ($role == 4) {
                    ...
                    if (!empty($ward_data) || !empty($category_data)) {

                        $data['projectsupport'] = $this->atr_report_model->get_project_support($area, $ward_data, $category_data, $plStaffId = '', $regionid = '', $subregionid);
                        $dataprojectsupport = $this->atr_report_model->get_project_support($area, $ward_data, $category_data, $plStaffId = '', $regionid = '', $subregionid);
                    } else {
                ...
                $data_action_takersupport = array();
                if (count($data['action_taker']) > 0) {
                    foreach ($data['action_taker'] as $val) {
                        $data_action_takersupport[] = $val['staffid'];
                    }
                }
                $subregion_data = (!empty($post_data['subregion'])) ? $post_data['subregion'] : '';
                $ward_data = (!empty($post_data['ward'])) ? $post_data['ward'] : '';
                    $category_data = (!empty($post_data['category'])) ? $post_data['category'] : '';
                $data['projectsupport'] = $this->atr_report_model->get_project_support($area, $ward_data, $category_data, $data_action_takersupport, $regionid, $subregion_data);
            ...
            if ($role == 3) {
                ...
                $data['subregion'] = $this->atr_report_model->get_subregion($area, $regionid, '', $staffid = '',$organizationid);
            } else {
                $data['subregion'] = $this->atr_report_model->get_subregion($area, $regionid, '', $staffid = '');
            }
            ...
            if ($role == 8) {
                ...
                $data['ward'] = $this->atr_report_model->get_wards($area,$subregionid, $wardid = '', $staffid = '', $regionid,$organizationid);
            }else {
                $data['ward'] = $this->atr_report_model->get_wards($area, $subregionid, $wardid = '', $staffid = '', $regionid);
            }
        ...
        $tableParams = [
            ...
            'durationto_date' => (!empty($post_data['durationreport-to'])) ? $post_data['durationreport-to'] : '',
            'durationfrom_date' => (!empty($post_data['durationreport-from'])) ? $post_data['durationreport-from'] : '',
    ...
    public function atr_role_wise_report()
    {
        ...
        $post_data = (!empty($this->input->post('duration'))) ? $this->input->post('duration') : '';
        $durationto_date1 = (!empty($this->input->post('durationreport-to'))) ? $this->input->post('durationreport-to') : '';
        $durationfrom_date1 = (!empty($this->input->post('durationreport-from'))) ? $this->input->post('durationreport-from') : '';
        $durationto_date = (!empty($this->input->post('durationreport-to'))) ? date("Y-m-d", strtotime($this->input->post('durationreport-to'))) : '';
        $durationfrom_date = (!empty($this->input->post('durationreport-from'))) ? date("Y-m-d", strtotime($this->input->post('durationreport-from'))) : '';
        $org = $this->input->post('organization');
        $department = $this->input->post('department');
        $staffid = $GLOBALS['current_user']->staffid;
         //pre($durationto_date.'--'.$durationfrom_date);
        ...
        // }
        if ($role == 8) {
            $data['atr_role_project_support'] = $this->atr_report_model->atr_rolewise_report(8, $area, '', $staffid, $post_data, $org, $department,$durationto_date,$durationfrom_date);
            $data['atr_role_project_leader'] = $this->atr_report_model->atr_rolewise_report(3, $area, $staffid, '', $post_data, $org, $department,$durationto_date,$durationfrom_date);
            ...
            $data['atr_rolewise_report'] = $this->atr_report_model->atr_rolewise_report(4, $area, '', $revlist, $post_data, $org, $department,$durationto_date,$durationfrom_date);
        } else if ($role == 3) {
            $data['atr_role_project_leader'] = $this->atr_report_model->atr_rolewise_report(3, $area, '', $staffid, $post_data, $org, $department,$durationto_date,$durationfrom_date);
            ...
            if (!empty($ps)) {
                foreach ($ps as $psval) {
                    $pslist[] = $psval['staff_id'];
                }
            }
            $data['atr_role_project_support'] = $this->atr_report_model->atr_rolewise_report(8, $area, '', $pslist, $post_data, $org, $department,$durationto_date,$durationfrom_date);
        } else if ($role == 4) {
            $data['atr_rolewise_report'] = $this->atr_report_model->atr_rolewise_report(4, $area, '', $staffid, $post_data, $org, $department,$durationto_date,$durationfrom_date);
            $pl = $this->staff_model->get_staff_projectleader($staffid);
            if (!empty($pl)) {
                foreach ($pl as $plval) {
                    $revlist[] = $plval['staff_id'];
                }
            }

            $data['atr_role_project_leader'] = $this->atr_report_model->atr_rolewise_report(3, $area, '', $revlist, $post_data, $org, $department,$durationto_date,$durationfrom_date);
            ...
            $data['atr_role_project_support'] = $this->atr_report_model->atr_rolewise_report(8, $area, '', $pslist, $post_data, $org, $department,$durationto_date,$durationfrom_date);
            $data['atr_role_project_leader'] = $this->atr_report_model->atr_rolewise_report(3, $area, '', $pslist, $post_data, $org, $department,$durationto_date,$durationfrom_date);
            $data['atr_rolewise_report'] = $this->atr_report_model->atr_rolewise_report(4, $area, '', $pslist, $post_data, $org, $department,$durationto_date,$durationfrom_date);
        }
        $tableParams['durationfrom_date'] = $durationfrom_date1;
        $tableParams['durationto_date'] = $durationto_date1;
        $tableParams['duration'] = $post_data;
    ...
    public function get_atr_projects()
    {
        ...
        $tableParams = [
            ...
            'statusIds' => $post_data_ticket,
            // 'duration' => (!empty($range)) ? $range : '',
            'duration' => (!empty($post_data2['duration'])) ? $post_data2['duration'] : '',
            'durationto_date' => (!empty($post_data2['durationreport-to'])) ? $post_data2['durationreport-to'] : '',
            'durationfrom_date' => (!empty($post_data2['durationreport-from'])) ? $post_data2['durationreport-from'] : '',
        ...
        foreach ($projects as $k => $val) {
            ...
            if($val['status'] == 10) $longterm = 'Yes'; else $longterm = 'No';
            ...
            if (!empty($assignedUserDetails->full_name) && !empty($assignedUserDetails->organisation)) {
                $assign = $assignedUserDetails->full_name . " (" .$assignedUserDetails->designation."-".$ticketDetails->sub_region_name."-". $assignedUserDetails->organisation . ")";
            } else {
                $assign = '';
            }
            ...
            $projects_data_array[$k]['ward_id'] = $val['ward_id'];
            ...
            $projects_data_array[$k]['organisation_id'] = $val['organisation_id'];
            ...
            $projects_data_array[$k]['user_type'] = (!empty($val['citizen_status']) ? $val['citizen_status'] : '');
    ...
    public function get_atr_projects_mapviewss_bkp()
    {
        $locInfo = [];
        $locationMarkers = [];
        $post_data = $this->input->post();
        ...
        $tableParams = [
            'areaid' => (!empty($post_data2['areaid'])) ? $post_data2['areaid'] : '',
            'region' => (!empty($post_data2['region'])) ? $post_data2['region'] : $region_at,
            'subregion' => (!empty($post_data2['subregion'])) ? $post_data2['subregion'] : $subregion_at,
            'category' => (!empty($cats)) ? $cats : '',
        ...
        //     if ($this->isArrayCompletelyEmpty($tableParams)) {
//         //echo "The array is completely empty";
//         //echo json_encode( ['success' => false,'locationMarkers' => "", 'locInfo' => "",'kml_file'=>""] );
//         exit;
//    } else {
//         echo "The array is not completely empty";
//     }
        $tableParams['limit_start'] = (!empty($post_data['start']) ? $post_data['start'] : 0);
        $tableParams['record_length'] = (!empty($post_data['length']) ? $post_data['length'] : 100);
        $projects = $this->atr_report_model->get_report($tableParams);
    ...
    public function get_atr_projects_mapviewss_110624bkp()
    {
        $locInfo = [];
        $locationMarkers = [];
        $post_data = $this->input->post();
...
//     }
        $tableParams['limit_start'] = (!empty($post_data['start']) ? $post_data['start'] : 0);
        $tableParams['record_length'] = (!empty($post_data['length']) ? $post_data['length'] : '');
        $projects = $this->atr_report_model->get_report_mapview($tableParams);


        //get project count
        $tableParams['total_records'] = true;
        //$project_count = $this->atr_report_model->get_report_mapview($tableParams);
        ...
        foreach ($projects as $k => $val) {
           ...
           // if ($arr_files_data) {
                ...
                $img = '';
                $file = 'uploads/projects/' . $project_id . '/' . $val['thumbnail_link'];
                if (file_exists($file)) {
                ...
                //$locationMarkers[] = [$ticketDetails->project_name, $arr_files_data->latitude, $arr_files_data->longitude, $status_color];
                $locationMarkers[] = [$val['project_name'], $val['latitude'], $val['longitude'], $status_color];
                $locInfo[] = [
                    "<div class=ticket_details data-project_id=" . $project_id
                    . " ><h4> Project Id : " . $project_id . "</h4><h4>" . $val['project_name'] . "</h4><p>(" . $val['landmark'] . ")</p><p style=text-align:center;>" . $img . "</p></div>"
                ];
            //}
            // google map end
...
//     }
        ...
        if (count($data) > 0) {
            ...
            foreach ($data as $rs) {
                ...
                if (!empty($status_name))
                    ...
                    $row[] = "";
                if (!empty($assignedUserDetails->firstname))
                    $row[] = $assignedUserDetails->firstname . " " . $assignedUserDetails->organisation;
                else
                    $row[] = "";
                if (!empty($assignedUserDetails->phonenumber))
                    $row[] = $assignedUserDetails->phonenumber;
                else
                    $row[] = "";
                if (!empty($rs['deadline']))
                    $row[] = stripslashes($rs['deadline']);
                else
                    $row[] = "";
                if (!empty($ticketDetails->sub_region_name))
                    $row[] = $ticketDetails->sub_region_name;
                else
                    $row[] = "";
                if (!empty($rs['landmark']))
                    $row[] = stripslashes($rs['landmark']);
                else
                    $row[] = "";
                if (!empty($ticketDetails->region_name))
                    $row[] = $ticketDetails->region_name;
                else
                    $row[] = "";
                if (!empty($ticketDetails->area_name))
                    $row[] = $ticketDetails->area_name;
                else
                    $row[] = "";
                if (!empty($rs['project_created']))
                    $row[] = stripslashes($rs['project_created']);
                else
                    $row[] = "";
                if (!empty($rs['description']))
                    ...
                    $row[] = "";
                if (!empty($evidence[0]))
                    $row[] = stripslashes($evidence[0]);
                else
                    $row[] = "NA";
                if (!empty($location[0]))
                    $row[] = stripslashes($location[0]);
                else
                    $row[] = "NA";
                if (!empty($projectNote_content))
                    $row[] = stripslashes($projectNote_content);
                else
                    $row[] = "";

                if (!empty($resolved_evidence))
                    $row[] = stripslashes($resolved_evidence);
                else
                    $row[] = "";
                if (!empty($resolved_location))
                    $row[] = stripslashes($resolved_location);
                else
                    $row[] = "";
                ...
                if (!empty($assignedUserDetails->role_name))
                    $row[] = $assignedUserDetails->role_name;
                else
                    $row[] = "";
                if (!empty($assignedUserDetails->email))
                    $row[] = $assignedUserDetails->email;
                else
                    $row[] = "";
                ...
                if (!empty($rs['date_finished'] && $rs['date_finished'] != '0000-00-00 00:00:00'))
                    $row[] = stripslashes(date("d-m-Y", strtotime($rs['date_finished'])));
                else
                    $row[] = "";
                if (!empty($user_type))
                    $row[] = stripslashes($user_type);
                else
                    $row[] = "";
                if (!empty($raised_name))
                    $row[] = stripslashes($raised_name);
                else
                    $row[] = "";
                if (!empty($raised_phone))
                    $row[] = stripslashes($raised_phone);
                else
                    $row[] = "";
                if (!empty($raised_email))
                    $row[] = stripslashes($raised_email);
                else
                    $row[] = "";
        ...
        if (count($data) > 0) {
            ...
            foreach ($data as $rs) {
                ...
                if (!empty($rs['id']))
                    ...
                    $row[] = "";
                if (!empty($rs['issue_name']))
                    $row[] = stripslashes($rs['issue_name']);
                else
                    $row[] = "";
                if (!empty($ticketDetails->status_name))
                    $row[] = $ticketDetails->status_name;
                else
                    $row[] = "";
                if (!empty($assignedUserDetails->firstname))
                    $row[] = $assignedUserDetails->firstname . " " . $assignedUserDetails->organisation;
                ...
                if (!empty($assignedUserDetails->email))
                    ...
                    $row[] = "";
                if (!empty($milestone['tag']))
                    $row[] = $milestone['tag'];
                else
                    $row[] = "";
                if (!empty($milestone['task_name']))
                    $row[] = $milestone['task_name'];
                else
                    $row[] = "";
                if (!empty($rs['date_finished']))
                    $row[] = stripslashes($rs['date_finished']);
                else
                    $row[] = "";
                if (!empty($rs['user_type']))
                    $row[] = stripslashes($rs['user_type']);
                else
                    $row[] = "";
                if (!empty($rs['firstname']))
                    $row[] = stripslashes($rs['firstname']);
                else
                    $row[] = "";
                if (!empty($rs['phonenumber']))
                    $row[] = stripslashes($rs['phonenumber']);
                else
                    $row[] = "";
                if (!empty($rs['email']))
    ...
    public function isArrayCompletelyEmpty($array)
    {
