...
class Atr_report_model extends App_Model
{
    ...
    public function get_wards($area_id = '', $sub_region = '', $ward_id = '')
    {
        $this->db->select('id, ward_name')->from(db_prefix() . 'manage_ward')->where('status', '1')->where('	is_deleted', '0');
        if ($area_id) {
            $this->db->where('area_id', $area_id);
        }
        if ($sub_region) {
    ...
    public function get_action_taker($area_id = '', $ward_id)
    {
