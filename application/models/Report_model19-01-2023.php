...
class Report_model extends App_Model
{
    ...
    public function get_project_leader($area_id ='', $ward_id = '') {
    ...
    public function get_project_leader_organization($area_id ='', $organization_id = '') {
        $this->db->select('CONCAT(s.firstname, " (", s.organisation, ")") AS name, s.staffid, s.organisation');
        $this->db->from('roles r');
        $this->db->join('staff s', 's.role = r.roleid AND r.slug_url = "at" ');
        $this->db->join('staff_region sr', 's.staffid = sr.staff_id');
        ...
        if ($organization_id) {
            $this->db->where_in('s.organisation', $organization_id);
        }
        $this->db->where("s.active", 1);
        $this->db->order_by("name", "asc");
        $query = $this->db->get();
    ...
    public function get_project_reviewer($area_id ='', $ward_id = '') {
        $this->db->select('CONCAT(s.firstname, " (", s.organisation, ")") AS name, s.staffid, s.organisation');
        $this->db->from('roles r');
    ...
    public function get_project_reviewer_organization($area_id ='', $ward_id = '') {
        $this->db->select('CONCAT(s.firstname, " (", s.organisation, ")") AS name, s.staffid, s.organisation');
        $this->db->from('roles r');
        $this->db->join('staff s', 's.role = r.roleid AND r.slug_url = "ar" ');
        $this->db->join('staff_region sr', 's.staffid = sr.staff_id');
        ...
        if ($ward_id) {
            $this->db->where_in('s.organisation', $ward_id);
        }
        $this->db->where("s.active", 1);
        $this->db->order_by("name", "asc");
        $query = $this->db->get();
