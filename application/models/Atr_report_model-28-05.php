...
class Atr_report_model extends App_Model
{
    ...
    public function get_wards($area_id = '', $sub_region = '', $ward_id = '', $staff_id = '', $region = '')
    {
    ...
    public function atr_rolewise_report($role = '', $area_id = '', $staffidpl = '',$staffidrev ='',$report_date ='',$org ='',$department ='')
    {
        if ($role == 4) {
            $roleslag = 'ar';
        } else if ($role == 3) {
        ...
        $this->db->select('CONCAT(s.firstname, " (", s.organisation, ")") AS name, s.staffid,SUM(CASE WHEN p.status IN(1,3,5,6,9,10,11,12,13,15,16) THEN 1 ELSE 0  END) as total,
            SUM(CASE WHEN (p.status = 1 AND p.frozen = 0) THEN 1 ELSE 0  END) as new,
            SUM(CASE WHEN (p.status = 3 AND p.frozen = 0) THEN 1 ELSE 0  END) as close, 
            SUM(CASE WHEN (p.status = 6 AND p.frozen = 0) THEN 1 ELSE 0  END) as reopen, 
            SUM(CASE WHEN (p.status = 5 AND p.frozen = 0) THEN 1 ELSE 0  END) as referred,
