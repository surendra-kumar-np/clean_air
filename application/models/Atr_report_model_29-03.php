...
class Atr_report_model extends App_Model
{
    ...
    public function get_wards($area_id = '', $sub_region = '', $ward_id = '', $staff_id = '', $region = '')
    {
        ...
        if ($ward_id) {
            $this->db->where('manage_ward.id', $ward_id);
        }
    ...
    public function get_action_taker($area_id = '', $ward_id = '', $staff_id = '')
    {
    ...
    public function get_report($tableParams)
    {
        ...
        $this->db->select('projects.frozen,projects.reassigned,projects.id,projects.sub_id,projects.status,projects.action_date,projects.name,projects.description,projects.landmark,projects.project_created,projects.sub_ticket_id, contacts.firstname,contacts.email,contacts.phonenumber,contacts.is_cc ,projects.is_assigned,projects.deadline, projects.date_finished,projects.rname,projects.rphonenumber,projects.remail,projects.view, IF(contacts.is_cc = 1 , "Call-Center" , "Surveyor") as user_type');
        ...
        if (!empty($bug)) {
            ...
            $this->db->where('(projects.name LIKE "%' . $bug . '%" OR projects.description = "' . $bug . '" OR projects.landmark ="' . $bug . '" OR projects.address = "' . $bug . '"OR project_status.name LIKE "%' . $bug . '%" OR staff.firstname like "%' . $bug . '%" OR project_notes.content LIKE "%' . $bug . '%")');
            }
            //}
        }
        if (!empty($report_date)) {
