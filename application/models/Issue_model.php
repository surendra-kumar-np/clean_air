...
class Issue_model extends App_Model
{
    ...
    public function get_area_issues($area_id, $active = 0,$staff_id="",$region_id="",$organisation_id='')
    {
        ...
        if(!empty($organisation_id)){
            $this->db->where("i.organization_id",$organisation_id);
        }
        if(!empty($region_id) ){
            if(is_array($region_id)){
                foreach($region_id as $val){
                    $this->db->like('i.region_id', $val, 'both'); 
                }
            }
            if(!is_array($region_id)){
                
                    $this->db->like('i.region_id', $region_id, 'both'); 
            }
        ...
        if(!empty($staff_id)){
            $this->db->join('staff_issues', 'staff_issues.issue_id = i.id');
            $this->db->join('staff_assistance sa', 'sa.staff_id = staff_issues.staff_id');
             $this->db->where_in('sa.assistant_id', $staff_id);
            $this->db->or_where_in('staff_issues.staff_id', $staff_id);
         }
    ...
    public function get_existing_area_issues_with_ward($ward_id="", $sub_region_id="", $region_id="")
    {
        $this->db->select('si.issue_id');
        $this->db->from('staff_region as sr');
        $this->db->join('staff_issues as si', "si.staff_id = sr.staff_id", 'LEFT');
