...
class Staff_region_model extends App_Model
{
    ...
    public function get_staff_region($id = false)
    {
        ...
        if(!$data){
            if (is_numeric($id)) {
                // $this->db->select("sr.*,sr.region as region_id, r.region_name as region_name, sreg.region_name as subregion_name, GROUP_CONCAT(r.region_name) as region, GROUP_CONCAT(sreg.region_name) as sub_region");
                ...
                $this->db->select("sr.*,mw.ward_name,sr.region as region_id, r.region_name as region_name, sreg.region_name as subregion_name, GROUP_CONCAT(r.region_name) as region, GROUP_CONCAT(sreg.region_name) as sub_region");
                $this->db->where('staff_id', $id);
                $this->db->from('staff_region sr');
                $this->db->join('region r', 'sr.region = r.id ', 'left');
                $this->db->join('sub_region sreg', 'sr.sub_region = sreg.id ', 'left');
                $this->db->join('manage_ward mw', 'sr.ward = mw.id ', 'left');
                $data = $this->db->get()->row();
                $this->app_object_cache->set('staff-region-data-' . $id, $data);
                return $data;
            }
