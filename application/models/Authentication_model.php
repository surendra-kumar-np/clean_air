...
class Authentication_model extends App_Model
{
    ...
    public function login($email, $password, $remember, $staff)
    {
        ...
        if ((!empty($email)) and (!empty($password))) {
			...
			if ($staff == true) {
				...
				$user_area = $user->area_id;
			}

            if($user_area != 0){
                $area = $this->db->where('areaid',$user_area)->get('area')->row();
                $status = $area->status;
            }
            ...
            if ($staff == true) {
				...
				$userData = $this->db->get('clients')->row();
    ...
    public function login_citizen($email, $password, $remember, $staff)
    {
        $status = 1;
        if ((!empty($email)) and (!empty($password))) {
            $table = db_prefix() . 'contacts';
            ...
            $this->db->where('email', $email);
            $this->db->where('email_verification_key', null, false);
            $this->db->order_by('active', 'DESC');
            $user = $this->db->get($table)->row();
    ...
    public function otp_login_citizen($email, $password, $remember, $staff)
    {
        $status = 1;
        if ((!empty($email))) {
            $table = db_prefix() . 'contacts';
    ...
    public function getcitizenphonenumber($id){
        $table = db_prefix() . 'projects';
        $this->db->select('clientid');
        $this->db->where('id', $id);
        $clientid = $this->db->get($table)->row()->clientid;
        if(!empty($clientid)){
            $tables = db_prefix() . 'contacts';
            $this->db->select('phonenumber');
            $this->db->where('id', $clientid);
            return $this->db->get($tables)->row()->phonenumber;
        }
        return false;
    }
    public function citizen_email_verification($id){
        $table = db_prefix() . 'contacts';
        $this->db->select('email_verification_key,phonenumber');
        $this->db->where('id', $id);
        $vpdateverificationcode = $this->db->get($table)->row()->email_verification_key;
        $mobileverificationcode = $this->db->get($table)->row()->phonenumber;
        if($vpdateverificationcode){
            $this->db->where('email_verification_key', $vpdateverificationcode);
            $this->db->update(db_prefix() . 'contacts', ['email_verification_key' => NULL, 'email_verified_at' => date('Y-m-d H:i:s')]);
            $validate =  $mobileverificationcode;
        }else{
            $validate = false;
        }
         return $validate;
    ...
    public function insert_otp_verifi($data) {
        $this->db->select('mobile')->from(db_prefix() . 'citizen_otp_verification');
            $this->db->where_in('mobile', $data['mobile']);
            $query = $this->db->get();
            $result = $query->result_array();
            $mobile_numbers = array_column($result, 'mobile');
            if(!empty($mobile_numbers)){
                $this->db->where_in('mobile', $mobile_numbers);
                $this->db->update(db_prefix() . 'citizen_otp_verification', ['otp' =>$data['otp']]);
            }else{
                return $this->db->insert('citizen_otp_verification', $data);
            }
        
    }
    public function citizenotpNumber($otp){
        if ($otp) {
            $this->db->select('otp,mobile')->from(db_prefix() . 'citizen_otp_verification');
            $this->db->where_in('otp', $otp);
            $query = $this->db->get();
            $result = $query->result_array();
            $mobile_numbers = array_column($result, 'otp');
            $mobile_numbersforverification = array_column($result, 'mobile');
            if(!empty($mobile_numbers)){
                $this->db->where_in('otp', $mobile_numbers);
                $this->db->update(db_prefix() . 'citizen_otp_verification', ['otp' =>'']);
                return $mobile_numbersforverification;
            }else{
                return 2;
            }
       ...
       public function check_mobilenumber($otp){
        if ($otp) {
            $this->db->select('phonenumber')->from(db_prefix() . 'contacts');
            $this->db->where_in('phonenumber', $otp);
            $query = $this->db->get();
            $result = $query->result_array();
            // echo $this->db->last_query();exit;
            $mobile_numbers = array_column($result, 'phonenumber');
            if(!empty($mobile_numbers)){
                return 1;
            }else{
                return 2;
       ...
       public function check_emailid($otp){
        if ($otp) {
            $this->db->select('email')->from(db_prefix() . 'contacts');
            $this->db->where_in('email', $otp);
            $query = $this->db->get();
            $result = $query->result_array();
            // echo $this->db->last_query();exit;
            $mobile_numbers = array_column($result, 'email');
            if(!empty($mobile_numbers)){
                return 1;
            }else{
                return 2;
    ...
    public function getotpNumber($phonenumber){
        if ($phonenumber) {
            $this->db->select('phonenumber')->from(db_prefix() . 'contacts');
            $this->db->where_in('phonenumber', $phonenumber);
            $query = $this->db->get();
            $result = $query->result_array();
            $mobile_numbers = array_column($result, 'phonenumber');
            if(!empty($mobile_numbers)){
                $randomNumber = mt_rand(100000, 999999);
                $this->db->where_in('phonenumber', $mobile_numbers);
                $this->db->update(db_prefix() . 'contacts', ['otp' =>$randomNumber]);
                return $randomNumber;
            }else{
                return 2;
            }
       ...
       public function get_phone_number($user_id) {
        ...
        $this->db->where('id', $user_id);  // Assuming 'id' is the column for user ID
        $query = $this->db->get();
        
        // Check if the result is found
        if ($query->num_rows() > 0) {
            // Return the phone number
            return $query->row()->phonenumber;
        } else {
            return false; // Return false if no phone number is found
        }
    }
       public function checkmobileNumber($phonenumber){
        if ($phonenumber) {
            $this->db->select('phonenumber')->from(db_prefix() . 'contacts');
            $this->db->where_in('phonenumber', $phonenumber);
            $query = $this->db->get();
    ...
    public function getotpNumberlogin($phonenumber){
        if ($phonenumber) {
            $this->db->select('email,otp')->from(db_prefix() . 'contacts');
            $this->db->where_in('otp', $phonenumber);
            $query = $this->db->get();
            $result = $query->result_array();
            $email_numbers = array_column($result, 'email');
            $otp_numbers = array_column($result, 'otp');
            if(!empty($otp_numbers)){
                $this->db->where_in('otp', $otp_numbers);
                $this->db->update(db_prefix() . 'contacts', ['otp' =>'']);
    ...
    public function citizen_email_verification_resend($id){
        $table = db_prefix() . 'contacts';
        $this->db->select('email,userid,id');
        $this->db->where('email', $id);
        $vpdateverificationcode = $this->db->get($table)->row();
        if($vpdateverificationcode){
            $this->db->where('email', $id);
            $this->db->update(db_prefix() . 'contacts', ['email_verification_key' => app_generate_hash(), 'email_verification_sent_at' => date('Y-m-d H:i:s')]);
            $customerArray = [
                'email' => $vpdateverificationcode->email,
                'userid'=>$vpdateverificationcode->userid,
                'id' => $vpdateverificationcode->id
            ];
            $customerObject = (object) $customerArray;
            $template_classverification = 'Customer_contact_verification';
                send_mail_template(
                $template_classverification,
                $customerObject
                );
            $validate =  true;
        }else{
    ...
    public function forgot_password_surveyor($email, $staff = false)
    {
        $table = db_prefix() . 'contacts';
        $_id   = 'id';
        if ($staff == true) {
    ...
    //
    public function checkforstaff($staff){
        $this->db->where('email', $staff);
    ...
    public function checkforCitizen($userEmail){
		
		$user = $this->db->select('t1.userid')
				->from(db_prefix() . 'contacts as t1')
				->join(db_prefix() . 'clients as t2', 't1.userid = t2.userid' )
				->where('t1.email', $userEmail)
				->where('t2.company', 'Citizen - Citizen')
				->get()
				->row();
    ...
    public function checkforCitizencheck($id){
    ...
    public function forgot_password($email, $staff = false)
    {
        ...
        $user = $this->db->get($table)->row();
        if($user->area != 0){
            $area = $this->db->where('areaid',$user->area)->get('area')->row();
            $status = $area->status;
        }
        if ($user) {
            if ($user->active == 0 ) {
    ...
    public function reset_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $merge_fields = [];
            if ($staff == false) {
                $sent = send_mail_template('customer_contact_password_resetted', $user->email, $user->userid, $user->$_id,$originalpass);
            } else {
                $sent = send_mail_template('staff_password_resetted', $user->email, $user->$_id,$originalpass);
            }
