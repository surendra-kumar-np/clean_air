...
class Atr_report_model extends App_Model
{
    ...
    public function get_report($tableParams)
    {
        ...
        if (empty($statusIds) && empty($report_date)) {
            $this->db->where('projects.id IN (select id from ' . db_prefix() . 'projects where projects.status = 3 AND projects.frozen = 0)');
    ...
    public function get_report_summary($tableParams, $summ = "")
    {
        ...
        if (empty($statusIds) && empty($report_date)) {
            ...
            $this->db->where('p.id  IN (select id from ' . db_prefix() . 'projects where projects.status = 3)');
