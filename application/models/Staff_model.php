...
class Staff_model extends App_Model
{
    ...
    public function update_profile($data, $id)
    {
		...
		return [
                'success' => false,
                'message' => 'Error: Profie update error OR No change in data.'
            ];
    }
    ...
    public function change_password($data, $userid)
    {
		...
		$data['newpasswordr'] = cryptoJsAesDecrypt($_SESSION['random_salt'], $data['newpasswordr']);
    ...
    {
        $status = hooks()->apply_filters('before_contact_status_change', $status, $id);

        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'contacts', [
            'active' => $status,
        ]);
        log_activity('Surveyor Status Changed [id: ' . $id . ' - Status(Active/Inactive): ' . $status . ']');
        if ($this->db->affected_rows() > 0) {
            return true;
        }
        return false;
    ...
    public function update_project($p_id,$staff_id){
        $data = [
                    "is_assigned"=>1,
                    "status" => 1
                ];
    ...
    {
        $this->db->select('s.firstname as name, s.staffid, s.organisation,s.designation');
        $this->db->from('roles r');
        $this->db->join('staff s', 's.role = r.roleid AND r.slug_url = "ar" AND s.area = ' . $area_id);
        if($GLOBALS['current_user']->role_slug_url == 'ar'){
            $this->db->join('staff_region sr', 's.staffid = sr.staff_id AND sr.region = ' . $GLOBALS['current_user']->location['region_id']);
        }
    ...
    public function get_city_reviewers($area_id, $city_id)
    {
        $this->db->select('s.firstname as name, s.staffid, s.organisation');
        $this->db->from('roles r');
        $this->db->join('staff s', 's.role = r.roleid AND r.slug_url = "ar" AND s.area = ' . $area_id);
        $this->db->join('staff_region sr', 's.staffid = sr.staff_id  AND sr.region = ' . $city_id);
        $this->db->where("s.active = 1");
        $result = $this->db->get()->result_array();
        if (count($result) > 0)
            return $result;
    ...
    {
        $select_str = 's.staffid,s.firstname,s.email,s.organisation,s.phonenumber,CONCAT(firstname," ","") as full_name,sa.assistant_id,sr.region,sr.sub_region,sr.ward';
    ...
    public function getWard($staffId){
    ...
    //ward list
    public function get_ward_list($id)
    {
        $this->db->select("manage_ward.*");
        $this->db->from("manage_ward");
        $this->db->where("manage_ward.subregion_id", $id);
        $this->db->where("manage_ward.status = '1'");
    ...
    //ward list
    public function get_ward_list_by_org_dept($region,$department_id,$organisation_id)
    {
        $this->db->select("manage_ward.*");
        $this->db->from("manage_ward");
        if($region){
            $this->db->where("manage_ward.region_id", $region);
        }
        if($organisation_id){
            $this->db->where("manage_ward.organisation_id", $organisation_id);
        }
        if($department_id){
            $this->db->where("manage_ward.department_id", $department_id);
        }
        $this->db->where("manage_ward.status = '1'");
        //$this->db->where('manage_ward.subregion_id',0);
        $this->db->order_by('manage_ward.ward_name', 'ASC');
        $result = $this->db->get()->result_array();
        return $result;
    }

    public function get_ward_list_by_org_dept_existing_id($region,$department_id,$organisation_id,$editId)
    {
        $this->db->select("manage_ward.*");
        $this->db->from("manage_ward");
        if($region){
        ...
        $this->db->where("manage_ward.status = '1'");
        $this->db->where('manage_ward.subregion_id',$editId);
        $this->db->order_by('manage_ward.ward_name', 'ASC');
        $result = $this->db->get()->result_array();
        return $result;
    }

    public function get_ward_list_by_org_dept_nonexist($region,$department_id,$organisation_id)
    {
        $this->db->select("manage_ward.*");
        $this->db->from("manage_ward");
        if($region){
        ...
        $this->db->where("manage_ward.status = '1'");
        $this->db->where('manage_ward.subregion_id',0);
        $this->db->where('manage_ward.subdivision_id',0);
        $this->db->order_by('manage_ward.ward_name', 'ASC');
        $result = $this->db->get()->result_array();
        return $result;
    }
    ...
    public function get_action_reviewer_action_takers($id)
    {
        ...
        $this->db->join("region as r", "r.id = sr.region", "left");
        $this->db->join("sub_region as srg", "srg.id = sr.sub_region", "left");
        $this->db->where_in("sa.assistant_id", $id);
        // $this->db->where("s.active = '1'");
        $this->db->order_by('sa.created_at', 'DESC');
        $result = $this->db->get()->result_array();
    ...
    {
        $select_str = 's.staffid,s.firstname,s.email,s.organisation,s.designation, s.phonenumber,CONCAT(s.firstname," ","") as full_name,sa.assistant_id,sr.region,sr.sub_region';
    ...
    public function get_staff_assistance_array($id)
    {
        ...
        $this->db->join('staff s', 's.staffid = sa.staff_id', 'left');
        $this->db->join('staff_region sr', 's.staffid = sr.staff_id ', 'left');
        $this->db->where('sa.assistant_id IN ('. $id .')');
        $this->db->group_by('s.staffid');
        $result = $this->db->get()->result_array();
        return $result;
    }

    public function get_staff_same_pl($id)
    {
        $pl = $this->get_reporting_person($id);
        
        $select_str = 's.staffid,s.firstname,s.email,s.organisation,s.designation,s.phonenumber,CONCAT(firstname," ","") as full_name,sa.assistant_id,sr.region,sr.sub_region';

        $this->db->select($select_str);
        $this->db->from('staff_assistance sa');
        $this->db->join('staff s', 's.staffid = sa.staff_id', 'left');
        $this->db->join('staff_region sr', 's.staffid = sr.staff_id ', 'left');
        $this->db->where('sa.assistant_id', $pl->staffid);
        $this->db->where_not_in('sa.staff_id', [$id]);
        $this->db->group_by('s.staffid');
        $result = $this->db->get()->result_array();
    ...
    {
        $select_str = 's.staffid,s.firstname,s.email,s.organisation,s.designation,s.phonenumber,CONCAT(firstname," ","") as full_name,s.area,GROUP_CONCAT(sr.region) as region ,GROUP_CONCAT(sr.sub_region) as sub_region,s.role,r.name as role_name,s.org_id,s.desig_id';
    ...
    public function updateOrgDeptStateWise($data){
        if($data['org_id']){
            $this->db->where('id', $data['org_id']);
            $this->db->update(db_prefix() . 'organization', ['state_id' => $data['area']]);
        }
    
        if($data['org_id'] && $data['desig_id']){
            foreach (json_decode($data['desig_id']) as $key => $desg_id) {
                $this->db->where('id', $desg_id );
                $this->db->where('org_id', $data['org_id']);
                $this->db->update(db_prefix() . 'department', ['state_id' => $data['area']]);
            }
    ...
    public function getReviewers($staffId){
        $this->db->select("assistant_id");
        $this->db->from("staff_assistance");
        $this->db->where("staff_id", $staffId);
        $reviewer = $this->db->get()->row();
    ...
    public function getReviewersfor_atr($staffId){
        $this->db->select("assistant_id");
        $this->db->from("staff_assistance");
    ...
    public function get_staff_projectsupport($id)
    {
        $this->db->select("staff_id");
        $this->db->from("staff_assistance");
        $this->db->where_in("assistant_id", $id);
        $ps = $this->db->get()->result_array();
        return $ps;
    }

    public function get_staff_projectleader($id)
    {
        $this->db->select("staff_id");
        $this->db->from("staff_assistance");
        $this->db->where_in("assistant_id", $id);
        $pl = $this->db->get()->result_array();
        // echo $this->db->last_query();exit;
        return $pl;
    }
    public function get_staff_orgid($id)
    {
    ...
    public function get_member_serveyor($data)
    {
        $this->db->where($data);
