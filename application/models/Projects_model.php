...
class Projects_model extends App_Model
{
        ...
        if ($insert_id) {
            ...
            if ($project_members > 0) {
                ...
                if (!empty($GLOBALS['current_user']->role_slug_url) && $GLOBALS['current_user']->role_slug_url == 'ar') {
                    ...
                    $this->log_activity($insert_id, 'ticket_assigned_to_ata', get_staff_full_name(get_staff_user_id()), json_encode($additional_data));
                } else {
                    $additional_data = array(
                        'logged_by' => $data["clientid"],
                        'assigned_to' => $project_members,
                    ...
                    $this->log_activity($insert_id, 'ticket_assigned_to_ata', $project_members, json_encode($additional_data));
                    //$this->load->model('Authentication_model'); 
			// $client_phone = $this->Authentication_model->getcitizenphonenumber($insert_id);
                    
            //         $templateId =1007172413646862622;
            //         $datecurrent = date('Y-m-d');
            //         getSmshelper($client_phone,"Hello Citizen. You have raised a complaint on the Bihar Clean Air App. Ticket ID ".$insert_id." has been generated on ".$datecurrent." date under the heading (Unpaved roads). The ticket has been assigned to ".get_staff_user_id()." and will be resolved by ".$actiondate." date. Team, CAD, Bihar",$templateId);
                }
            } else {

                $state_admin = get_area_admin_by_area($data['area_id']);

                $this->db->insert(db_prefix() . 'project_members', [
                    'project_id' => $insert_id,
                    'staff_id'   => $state_admin,
                ]);

                $additional_data = array(
                    'logged_by' => $data["clientid"],
                    'assigned_to' => $state_admin,
                    'taskId' => '',
                    'status' => 9,
                    'comment' =>''
                );
                ...
                $this->log_activity($insert_id, 'No Project Support assigned to <strong>' . getcatname($data['issue_id']) . '</strong> in <strong>' . getitemname($data['ward_id'], 'ward') . '</strong> Raised to State Admin <strong>' . get_staff_full_name(get_area_admin_by_area($data['area_id'])) . '</strong>', get_area_admin_by_area($data['area_id']), json_encode($additional_data), 1, "unassigned");
    ...
    public function log_activity($project_id, $description_key, $staffid = 0, $additional_data = '', $visible_to_customer = 1, $check = '')
    {
        if ($check != '') {
            ...
            if (!DEFINED('CRON')) {
                if (is_client_logged_in()) {
                    $data['contact_id'] = ($description_key == 'unassigned' or $description_key == 'ticket_assigned_to_ata') ? 0 : get_contact_user_id();
                    $data['staff_id']   = $staffid;
                    $data['fullname']   = ($description_key == 'unassigned' or $description_key == 'ticket_assigned_to_ata') ? get_staff_full_name($staffid) : get_contact_full_name(get_contact_user_id());
                } elseif (is_staff_logged_in()) {
                    $data['contact_id'] = 0;
                    $data['staff_id']   = get_staff_user_id();
                    $data['fullname']   = get_staff_full_name(get_staff_user_id());
    ...
    public function save_ticket_note($data, $project_id)
    {
        if(!empty($data['content'])){
            $this->db->insert(db_prefix() . 'project_notes', [
                ...
                'exception' => !empty($data['exception'])?$data['exception']:NULL
            ]);
            $insert_id = $this->db->insert_id();
            if ($insert_id) {
    ...
    {
        $select = "p.id as project_id,p.`name` as project_name, p.`status` as project_status, p.project_created as logged_date, p.*, a.`name` as area_name, r.region_name, sr.region_name as sub_region_name,c.company,conts.firstname, conts.lastname, conts.phonenumber, conts.email, ps.name as status_name, org.name as organization_name, ward.ward_name as ward_name,
        ...
        $this->db->join(db_prefix() . 'organization org ', 'org.id = p.organisation_id', 'left');
        $this->db->join(db_prefix() . 'manage_ward ward ', 'ward.id = p.ward_id', 'left');
        $this->db->join(db_prefix() . 'clients c ', 'c.userid = p.clientid', 'left');
        $this->db->join(db_prefix() . 'contacts conts ', 'conts.userid = p.clientid', 'left');
    ...
    public function get_project_notes($projectId)
    {
        $select = "pn.*";

        $this->db->select($select);
        $this->db->from(db_prefix() . 'project_notes pn');
        $this->db->where('pn.project_id', $projectId);
        $this->db->where('pn.status', 4);
        $this->db->order_by("pn.id", "DESC");

        return $this->db->get()->row();
    }

    public function get_project_notes_by_status($projectId,$status)
    {
        $select = "pn.*";
        ...
        $this->db->from(db_prefix() . 'project_notes pn');
        $this->db->where('pn.project_id', $projectId);
        $this->db->where('pn.status', $status);
        $this->db->order_by("pn.id", "DESC");

        return $this->db->get()->row();
    }

    public function get_project_details_ids($projectId)
    {
        $select = "p.id as project_id,p.`name` as project_name, p.`status` as project_status, p.project_created as logged_date, p.*, a.`name` as area_name, r.region_name, r.id as region_id, sr.region_name as sub_region_name, sr.id as subregion_id, w.ward_name, w.id as ward_id, p.issue_id, ic.issue_name, c.company,ps.name as status_name, ta.status as task_status, ta.taskid as update_task_id,
        pm.staff_id as assigned_user_id,pm.created_at as assigned_date, p.organisation_id, p.department_id, oz.name as oz_name, dt.depart_name";
        ...
        $this->db->join(db_prefix() . 'region r ', 'r.id = p.region_id', 'left');
        $this->db->join(db_prefix() . 'sub_region sr ', 'sr.id = p.subregion_id', 'left');
        $this->db->join(db_prefix() . 'manage_ward w ', 'w.id = p.ward_id', 'left');
        $this->db->join(db_prefix() . 'clients c ', 'c.userid = p.clientid', 'left');
        $this->db->join(db_prefix() . 'project_members pm ', 'pm.project_id = p.id', 'left');
        ...
        $this->db->join(db_prefix() . 'organization oz ', 'oz.id = p.organisation_id', 'left');
        $this->db->join(db_prefix() . 'department dt ', 'dt.id = p.department_id', 'left');
    ...
    public function get_task_longterm_milstones($project_id)
    {
        $this->db->select("t.id as task_id, t.name as task_name, t.startdate, t.duedate, t.datefinished, t.task_days, t.is_closed, t.status, ps.name as task_status,t.reminderone_days,t.remindertwo_days,t.rel_id");
        $this->db->from("tasks as t");
        $this->db->join("project_status as ps", "t.status = ps.id", "LEFT");
    ...
    public function updateAssignedUser($projectId, $staffId,$reAssignStatus = NULL, $comment = NULL)
    {
        ...
        if (!$_exists) {
            ...
            $this->db->where('project_id', $projectId);
            $this->db->where('staff_id', $staffId);
            $this->db->update(db_prefix() . 'project_members', ['assigned' => 1,'viewed' => 0, 'active' => 1, 'updated_at' => date('Y-m-d H:i:s')]);
        }
        ...
        $status = !empty($reAssignStatus)?$reAssignStatus:5;
        if ($role == 3) {
        ...
        if ($role == 3 && $status == 5) {
            $label = 'ticket_referred_to_at';
        } else if ($role == 4 && $status == 5) {
            $label = 'ticket_referred_to_ar';
        } else if ($role == 8 && $status == 5) {
            $label = 'ticket_referred_to_ata';
        } else if ($role == 3 && $status == 12) {
            $label = 'ticket_send_to_at_close_approval';
        } else if ($role == 8 && $status == 6) {
            $label = 'ticket_reopen_by_at_ps_assigned';
        } else if($role == 8 && $status == 10){
            $label = 'ticket_assigned_to_ata';
        }
    ...
    public function updateAssignedUserForRejected($projectId, $staffId,$reAssignStatus = NULL){
    ...
    public function reassigned_ticket($projectId, $assignAt, $extendDeadlineData, $milestoneCnt)
    {
        ...
        if (!empty($projectDueDate)) {
            ...
            $dlabel = 'Project due date changed from <strong>' . setDateFormat($projectDueDate) . '</strong> to <strong>' . setDateFormat($nw_deadline) . '</strong>  by ' .$actionBy. ' ';
            $additional_data = array(
                'assigned_by' => get_staff_user_id(),
                'assigned_to' => $assignAt,
                'taskId' => '',
                'status' => 1,
                'comment' => ''
            );
        ...
        $newProjectStatus = array(
            ...
            'date_finished' => NULL,
            'action_date' => $action_date,
            'status' => isset($projectStatus) ? $projectStatus : 1,
    ...
    public function getProjectAssignee($projectId)
    {
        ...
        $staff = $this->db->get(db_prefix() . 'project_members')->row();
