...
class Tickets_model extends App_Model
{
    ...
    public function updateTicketStatus($projectId, $userId, $staffId)
    {
        ...
        $update_team_members = $this->projects_model->update_team_members($data, $projectId);

        if ($update_team_members) {
            //Update Task Assigned
            $this->load->model('tasks_model');
            $this->tasks_model->assign_task_assignees($projectId, $staffId);
            ...
            $this->projects_model->mark_as($statusData);

            //Update project_notes

            //Update First milestone to WIP
            $this->load->model('tasks_model');
            $update_team_members = $this->tasks_model->update_task_status($projectId);

            //check if ticket has sub ticket
            $parentId = $this->projects_model->getSubChildTickets($projectId);
            if (empty($parentId)) {
                // Update Action Date
                $this->update_action_date($projectId);
            }else{
                // Update Parent Action Date
    ...
    //Update Action Date
    public function update_action_date($projectId)
    {
        //Get all milestone deadline
        $this->db->from(db_prefix() . "tasks");
        $this->db->where("status = 2 AND rel_id = " . $projectId);
        $this->db->order_by("startdate", "ASC");
        $task = $this->db->get();

        $due_date = '';
        if (!empty($task->result_id->num_rows)) {
            $num_rows = $task->result_id->num_rows;
            if ($num_rows == 1) {
                $result_array = $task->row();
                $due_date = $result_array->duedate;
            } elseif ($num_rows > 1) {
                $result_array = $task->result_array();
                if (!empty($result_array) && $result_array[0]['name'] == 'Project Closure') {
                    $due_date = $result_array[1]['duedate'];
                } else {
                    $due_date = $result_array[0]['duedate'];
                }
            }
            $this->db->set('action_date', $due_date);
        } else {
            $this->db->set('action_date', 'deadline', false);
        }
        $this->db->where("id", $projectId);
        $this->db->update(db_prefix() . "projects");
    }

    public function update_milestone($data)
    {
        $this->db->where("id", $data['task_id']);
        $this->db->update("tasks", ["datefinished" => date('Y-m-d H:i:s') ,"status" => $data['status']]);

        if ($this->db->affected_rows() > 0) {
            log_activity('Task status changed [ID:' . $data['task_id'] . ', status: ' . $data['status'] . ']');

            $this->db->select("id");
            $this->db->from("tasks");
            $this->db->where("status = 0 AND rel_id = " . $data["project_id"]);
            $this->db->order_by("startdate", "ASC");
            $task = $this->db->get()->row();

            if (isset($task)) {
                $this->db->where("id", $task->id);
                $this->db->update("tasks", ["status" => 2]);
                log_activity('Task status changed [ID:' . $task->id . ', status: 2]');
            } else {
                $this->db->where(["rel_id" => $data['project_id'], "rel_type" => "project", "status" => 2]);
                $this->db->update("tasks", ["status" => 4]);

                $this->db->where("id", $data['project_id']);
                $this->db->update("projects", ["status" => 12]);

                log_activity('Project status changed [ID:' . $data['project_id'] . ', status: 12]');
            }

            // Update Action Date
            $this->update_action_date($data['project_id']);

            $this->db->insert("project_notes", ["project_id" => $data['project_id'], "task_id" => $data['task_id'], "content" => $data["comment"], "staff_id" => $data['staff_id'], "created_at" => date('Y-m-d H:i:s'), "status" => 12]);
            $this->load->model("projects_model");
            $additional_data = json_encode([
                "assigned_by" => $data['staff_id'],
                "assigned_to" => $data['staff_id'],
                "taskId" => $data['task_id'],
                "status" => $data["pStatus"],
                "comment" => $data["comment"],
            ]);
            $this->projects_model->log_activity($data['project_id'], 'project_activity_updated_milestone', $data['staff_id'], $additional_data);
            return true;
        }
        return false;
    }

    public function update_ticket_verification($data)
    {
            $this->db->where("id", $data['project_id']);
            $this->db->update("projects", ["status" => $data['pStatus']]);

            log_activity('Project status changed [ID:' . $data['project_id'] . ', status: '.$data['pStatus'].']');

            // Update Action Date
            $this->update_action_date($data['project_id']);

            $this->db->insert("project_notes", ["project_id" => $data['project_id'], "task_id" => $data['task_id'], "content" => $data["comment"], "staff_id" => $data['staff_id'], "created_at" => date('Y-m-d H:i:s'), "status" => 13]);
            $this->load->model("projects_model");
            $additional_data = json_encode([
                "assigned_by" => $data['staff_id'],
                "assigned_to" => $data['staff_id'],
            ...
            $this->projects_model->log_activity($data['project_id'], 'project_activity_updated_verification', $data['staff_id'], $additional_data);
            return true;
        //}
        return false;
    }

    public function add_project_files($file_data)
    {
        $this->db->insert_batch("project_files", $file_data);

        if ($this->db->affected_rows() > 0) {
            log_activity('New Task Images Added [ID:' . $file_data[0]['task_id'] . ', count: ' . count($file_data) . ']');
            return true;
        }
        return false;
    }

    public function updateRejectTicketAssignment($projectId)
    {
        $userId = $GLOBALS['current_user']->staffid;
        $this->load->model('staff_model');
        $reportingPerson = $this->staff_model->get_reporting_person($userId)->staffid;
        //Assign Ticket to Team Member
        $data['project_members'] = array(
            'staff_id' => $reportingPerson
        );
        ...
        if ($update_team_members) {
            //Update Task Assigned
            $this->load->model('tasks_model');
            $this->tasks_model->assign_task_assignees($projectId, $reportingPerson);
            $this->projects_model->updateAssignedUser($projectId, $reportingPerson, 11);

            //Send new ticket assignment email to AR - for ticket rejected by AT
            $this->projects_model->sendAssignEmailNotification($projectId, $reportingPerson,'New_ticket');
    ...
    public function updateApproveTicketAssignment($projectId)
    {
        $userId = $GLOBALS['current_user']->staffid;
        $this->load->model('staff_model');
        // $reportingPerson = $this->staff_model->get_reporting_person($userId)->staffid;
        $reportingPerson = !empty(getProjectAssignedPreviousUser($projectId,'lt'))?getProjectAssignedPreviousUser($projectId,'lt'):'';
        //Assign Ticket to Team Member
        $data['project_members'] = array(
            'staff_id' => $reportingPerson
        );
        ...
        if ($update_team_members) {
            ...
            $this->load->model('tasks_model');
            $this->tasks_model->assign_task_assignees($projectId, $reportingPerson);
            $this->projects_model->updateAssignedUser($projectId, $reportingPerson);
    ...
    public function get_milstone_tasks($id, $limit=null)
    {
        if (is_numeric($id)) {
            $this->db->where('id', $id);
            //$this->db->where('rel_type', "longterm");
    ...
    public function get_milstone_tasks_first($id)
    {
        if (is_numeric($id)) {
    ...
    public function get_milstone_tasks_array($id)
    {
        $this->db->where('rel_id', $id);
        $query = $this->db->get(db_prefix() . 'tasks')->result_array();
        
        return $query;
    }

    public function get_tasks_array($id)
    {
        $this->db->where('rel_id', $id);
        $query = $this->db->get(db_prefix() . 'task_milestones')->result_array();
        
        return $query;
    }

    public function addmilestonetask($data)
    {
    ...
    public function delete_milstone_task($id){
        $this->db->where_in('id', $id);
        $this->db->delete(db_prefix() . 'tasks');
        return true;
    }

    public function delete_milstone_task_to_project($id){
        $this->db->where_in('rel_id', $id);
        $this->db->delete(db_prefix() . 'tasks');
        return true;
    }

    public function delete_milstone_task_backup($id){
        $this->db->where_in('rel_id', $id);
        $this->db->delete(db_prefix() . 'task_milestones');
        return true;
    }

    public function add_milstone_task_backup($row){
        if($row['id']){
            $row['task_id'] = $row['id'];
            unset($row['id']);
        }
        $this->db->insert(db_prefix() . 'task_milestones', $row);
        $insert_id = $this->db->insert_id();
        return $insert_id;
    }

    public function add_old_task($row){
        if($row['id']){
            unset($row['id']);
            $row['id'] = $row['task_id'];
            unset($row['task_id']);
        }
        $this->db->insert(db_prefix() . 'tasks', $row);
        $insert_id = $this->db->insert_id();
        return $insert_id;
    }
