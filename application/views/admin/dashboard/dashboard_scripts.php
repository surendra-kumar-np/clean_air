...
<script>
    ...
    $(document).on('click', '.loadWidgetData', function(e) {
    ...
    $(document).on('click', '.ticketVerified', function() {
        //console.clear();
        // alert(locationc);
        // let res2   =iosdevicevalidation();
        let res = validatetextarea();
        ...
        // }
        $.ajax({
            url: admin_url + "tickets/update_ticket_verified",
            data: form_data,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $(document).on('click', '.action_items_data', function() {
        let nextPage = 1;
        let nextPageget = $('#ajexpagination ul li.active').data('curitem');
        if(nextPageget > 1){
            nextPage = nextPageget;
        }
    ...
    $(document).on('click', '.action_items_data_refered', function() {
        let nextPage = 1;
        $('#actionItemsrefered').html('');
         loadActionDataRefered(nextPage);
    });
    $(document).on('click', '.action_items_data_pending_for_approval', function() {
        let nextPage = 1;
        let nextPageget = $('#ajexpaginationpendingforapproval ul li.active').data('curitem');
        if(nextPageget > 1){
            nextPage = nextPageget;
        }
        $('#actionItemspendingforapproval').html('');
        loadActionDataPendingforApproval(nextPage);
    });
            ...
            success: function(response) {
                if (response.success) {
                    if (response.action_items_cnt === 0 && response.message === '') {
                        $('.no-action-item-data').show();
                        $('.action-items-data').addClass('hide');

                    } else if (response.message != '') {
                        $('.no-action-item-data').hide();
                        $('.action-items-data').removeClass('hide');
                        $('#actionItems').html('');
                        
                        $('span.action_items_cnt1').html('<span class="action_items_cnt1">'+response.action_items_countview+'/</span>');
                        $('#actionItems').append(response.message);

                    }
                    $('span#thispageTablecount').html(response.action_items_cnt);
                    $('#pageno').val(nextPage);
            ...
            success: function(response) {
                if (response.success) {
                    if (response.action_items_refered_cnt === 0 && response.message === '') {
                        $('.no-action-item-data-refered').show();
                        $('.action-items-data-refered').addClass('hide');
                       
                    } else if (response.message != '' || response.message == '') {
                        $('.no-action-item-data-refered').hide();
                        $('.action-items-data-refered').removeClass('hide');
                        $('#actionItemsrefered').html('');
                        
                        $('span.action_items_refered_cnt').html('<span class="action_items_refered_cnt">'+response.action_items_refered_viewedcnt+'/</span>');
                        $('#actionItemsrefered').append(response.message);
                    }
                    $('span#thispageTablecountrefer').html(response.action_items_refered_cnt);
                    $('#pageno').val(nextPage);
                    let listCnt = (nextPage > 0) ? nextPage * action_item_list : action_item_list;
            ...
            success: function(response) {
                if (response.success) {
                    if (response.action_items_pendingforapproval_cnt === 0 && response.message === '') {
                        $('.load-btn-action-item_pendingforapproval').show();
                       // $('.load-btn-action-item-pending-for-approval').removeClass('hide');
                    } else if (response.message != '') {
                        $('.load-btn-action-item_pendingforapproval').hide();
                        //$('.load-btn-action-item-pending-for-approval').removeClass('hide');
                        $('#actionItemspendingforapproval').html('');
                        $('span#thispageTablecountpendingforapproval').html(response.action_items_pendingforapproval_cnt);
                        $('span.action_items_pendingforapproval_cnt').html('<span class="action_items_pendingforapproval_cnt">'+response.action_items_pendingforapproval_cntviewed+'/</span>');
                    ...
                    if (response.action_items_pendingforapproval_cnt > listCnt && response.message != '') {
                        ...
                        // if(response.action_items_cnt >= 1 && response.message != ''){
                            $('span#thispageTablecountpendingforapproval').html(response.action_items_pendingforapproval_cnt);
                            $('span.action_items_pendingforapproval_cnt').html('<span class="action_items_pendingforapproval_cnt">'+response.action_items_pendingforapproval_cntviewed+'/</span>');
                        $('.load-btn-action-item_pendingforapproval').addClass('hide');
                    } else {
                        $('.load-btn-action-item_pendingforapproval').removeClass('hide');
                    }
                } else {
                    $("#loadMoreAction").hide();
                }
    ...
    $(document).on('click', '.verified_data', function() {
        let nextPage = 1;
        $('#verifiedItems').html('');
        loadVerifiedData(nextPage);
    });

    $(document).on('click', '#verifiedDataLoader', function() {
        // let nextPage = isNaN(parseInt($('#nextpageno').val())) ? 2 : parseInt($('#nextpageno').val()) + 1;
        let nextPage = isNaN(parseInt($('#nextpageno').val())) ? 2 : parseInt($('#nextpageno').val()) + 1;
        loadVerifiedData(nextPage);
    });

    function loadVerifiedData(nextPage) {
        let action_item_list = $('.action_item_list').val();
        
        $.ajax({
            type: 'POST',
            url: admin_url + 'dashboard/verifiedData',
            data: {
                nextpageno: nextPage
            },
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    if (response.upcoming_deadline_data_cnt == 0 && response.message == '') {
                        $('.no-verified-data').removeClass('hide');
                        $('.verified-data').addClass('hide');
                    } else if (response.message != '') {
                        $('.no-verified-data').addClass('hide');
                        $('.verified-data').removeClass('hide');
                        $('#verifiedItems').html("");
                        $('#verifiedItems').append(response.message);
                    }
                    ...
                    if (response.upcoming_deadline_data_cnt > listCnt && response.message != '') {
                        // if(response.upcoming_deadline_data_cnt > action_item_list && response.message != ''){
                        // if(response.upcoming_deadline_data_cnt >= 1 && response.message != ''){
                        $('.load-btn-verified').removeClass('hide');
                    } else {
                        $('.load-btn-verified').addClass('hide');
                    }
                } else {
                    $("#verifiedDataLoader").hide();
                }
                loadWidgetData();
            }
        });
    ...
    $(document).on('click', '.ticket_details', function() {
        let activeLi = $('.dashboard-tab li.active');
        console.log(activeLi.attr('class'));
        $(this).parent('td').css('background-color', '#fff').siblings().css('background-color', '#fff');
        clicked_attr = $(this).parent('div').attr('style');
        console.log('clicked_attr',clicked_attr);
        $(this).closest('.dashboard-row').addClass('active-cell');
        let projectId = $(this).data('project_id');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                let nextPage =1;
                if(activeLi.attr('class') == 'action_items_data_pending_for_approval active'){
                    let nextPageget = $('#ajexpaginationpendingforapproval ul li.active').data('curitem'); 
                    if(nextPageget > 1){
                         nextPage = nextPageget;
                        }   
                    }else if(activeLi.attr('class') == 'action_items_data_refered active'){
                        
                        let nextPageget = $('#ajexpaginationrefer ul li.active').data('curitem'); 
                    if(nextPageget > 1){
                         nextPage = nextPageget;
                        }   
                    }else{
                ...
                if(clicked_attr!==undefined){
                    loadActionDataPendingforApproval(nextPage);
                    loadActionDataRefered(nextPage);
                    let viewedCount =1;
                    if(activeLi.attr('class') == 'action_items_data_pending_for_approval active'){
                        let viewedCount = $('.action_items_pendingforapproval_cnt .action_items_pendingforapproval_cnt').html().replace('/','');
                        if(viewedCount<0) viewedCount = 0
                        $('.action_items_pendingforapproval_cnt').find('.action_items_pendingforapproval_cnt').html(viewedCount+'/');
                    }else if(activeLi.attr('class') == 'action_items_data_refered active'){
                        let viewedCount = $('.action_items_refered_cnt .action_items_refered_cnt').html().replace('/','');
                        if(viewedCount<0) viewedCount = 0
                        $('.action_items_refered_cnt').find('.action_items_refered_cnt').html(viewedCount+'/');
                    }else{
                        let viewedCount = $('.action_items_cnt1 .action_items_cnt1').html().replace('/',''); 
                        if(viewedCount<0) viewedCount = 0
                        $('.action_items_cnt1').find('.action_items_cnt1').html(viewedCount+'/'); 
                    }
                    // let viewedCount = $('.action_items_cnt1 .action_items_cnt1').html().replace('/','');
                    // if(viewedCount==''){
                    ...
                    if(clicked_attr == 'background-color:#eaf1fb'){
                        viewedCount--;
                    }
                ...
                if(report == "report-history"){
                    $('#ticket-details').removeClass('active');
                    $('#ticket-details').removeClass('in');
                    $('.project-detail-list').removeClass('active');
                    $('.project-history-list').addClass('active');
                    $('#ticket-history').addClass('active');
                    $('#ticket-history').addClass('in');
                }
            }
        });
        ...
        // $('.project-detail-list').removeClass('active');
        // $('.project-history-list').addClass('active');
        // $('.project-history').trigger('click');
        
        // $(".project-detail-list").attr("aria-expanded","false");
        // $(".project-history-list").attr("aria-expanded","true");
        // $('#ticket-history').tab("show");
        // $('#ticket-history').show();
    ...
    function loadWidgetData() {
        $.ajax({
            ...
            success: function(response) {
                ...
                $('.longterm_total').html(data.longterm);
                $('.resolved_total').html(data.reopen);
                $('.submitforapproval_total').html(data.submitforapproval_total);
    ...
    $(document).on('click', '.markVerification', function() {
        let projectId = $('.ticketPopupId').val();
        let varifyStatus = $('.markVerificationList').find("option:selected").val();
        let varifyName = $.trim($('.markVerificationList').find("option:selected").text());
        sendVerification(projectId, varifyStatus, varifyName);
    });
    // $(document).on('click', '#ticketDetailsPopup .close', function(){$('.waitMe').css('display', 'none')});
    function sendVerification(projectId, varifyStatus, varifyName) {
        if (typeof varifyStatus === "undefined" || varifyStatus == '' || varifyStatus == null) {
            alert_float('danger', 'Please select any verify status.');
            return false;
        }
        $.ajax({
            type: 'POST',
            url: admin_url + 'tickets/verify_status',
            cache: false,
            data: {
                varifyStatus: varifyStatus,
                varifyName: varifyName,
                projectId: projectId
            },
            success: function(response) {
                let data = $.parseJSON(response);
    ...
    $(document).on('click', '.refer-btn-popup', function() {
        let projectId = $(this).data('projectid');
        $('#referTicketPopup').find('.assignTDListticker').val(projectId);
        $('#referTicketPopup').modal('show');
    });
    
    $(document).on('click', '.assignTDTicketlist', function() {
        let projectId = $('.assignTDListticker').val();
        let staffId = $('.dlist').find("li a.active_li").data('staffid');
        let staffName = $('.dlist').find("li a.active_li").text();
        let ticketDetail = $(this).data('ticketdetail');
        acceptTicket(projectId, staffId, staffName, ticketDetail);
        //console.log(projectId+ '--'+staffId+'--'+staffName+'--'+ticketDetail);
    });
    ...
    $(document).on('change', '.rejectTDList', function() {
        ...
        let exceptionId = $(this).val();
        
        let exceptionName = $.trim($(this).text());
        //console.log(exceptionId);
        if ($(this).prop('checked')==true) {
            $('.hide-longterm').removeClass('hide');
        } else {
            $('.hide-longterm').addClass('hide');
        }
    });
    ...
    $(document).on('click', '.rejectTDTicket', function() {
        ...
        if (exceptionName.toLowerCase() == 'long term request') {
        ...
        rejectTicket(projectId, exceptionId, exceptionName, ticketDetail, ticketDetail, extendDeadlineData, milestoneCnt);
    });

    function rejectTicket(projectId, exceptionId, exceptionName, ticketDetail, ticketDetail, extendDeadlineData, milestoneCnt) {
        if (typeof exceptionId === "undefined" || exceptionId == '') {
            alert_float('danger', 'Please give reason to longterm this project.');
            return false;
        }
    ...
    $(document).on('change', '.approveTDTicketlist', function() {
        ...
        // }
        if (exceptionId == 10) {
            $('.panel-border-top-reason').addClass('hide');
        } else {
            $('.panel-border-top-reason').removeClass('hide');
        }
    });
    ...
    //assigned to project support for ticket
    $(document).on('click', '.assigned-new-ps', function() {
        let projectId = $(this).data('projectid');
        let staffid = $(this).data('staffid');

        //$('.evidence-data').html('');
        if (confirm('Are you sure you want to assign this project?')) {
            $.ajax({
                type: 'POST',
                url: admin_url + 'dashboard/assigned_to_ps',
                data: {
                    projectId: projectId,
                    staffid: staffid
                },
...
<script>
    const element = document.querySelector("#ajexpagination ul");
    $("#ajexpagination").hide();
    ...
    function createPagination(totalPages, page) {
        ...
        if (page > 2) {
            liTag += `<li class="first numb" data-curitem="1" onclick="createPagination(totalPages, 1)"><span>1</span></li>`;
            if (page > 3) {
        ...
        for (var plength = beforePage; plength <= afterPage; plength++) {
            ...
            liTag += `<li class="numb ${active}" data-curitem="${plength}" onclick="createPagination(totalPages, ${plength})"><span>${plength}</span></li>`;
        }
        ...
        if (page < totalPages - 1) {
            ...
            liTag += `<li class="last numb" data-curitem="${totalPages}" onclick="createPagination(totalPages, ${totalPages})"><span>${totalPages}</span></li>`;
        }
...
<script>
    ...
    if($('#thispageTablecountrefer').html() != undefined || $('#thispageTablecountrefer').html() != null){
        $("#ajexpaginationrefer").hide();
    const refer = document.querySelector("#ajexpaginationrefer ul");
    
    var totalrowsrefer = $('#thispageTablecountrefer').html();
    
    var totalPagesrefer = Math.ceil(totalrowsrefer / 10);
    if(totalPagesrefer >1){
        $("#ajexpaginationrefer").show();
    let referpage = 1;

    refer.innerHTML = createPaginationRefer(totalPagesrefer, referpage);
 
    function createPaginationRefer(totalPagesrefer, referpage) {
        let liTagRef = '';
        let active;
        let beforePage = referpage - 1;
        let afterPage = referpage + 1;
        loadActionDataRefered(referpage);
        if (referpage > 1) {
            liTagRef += `<li class="btn prev" onclick="createPaginationRefer(totalPagesrefer, ${referpage - 1})"><span>Prev</span></li>`;
        }

        if (referpage > 2) {
            liTagRef += `<li class="first numb" data-curitem="1" onclick="createPaginationRefer(totalPagesrefer, 1)"><span>1</span></li>`;
            if (referpage > 3) {
                liTagRef += `<li class="dots"><span>...</span></li>`;
            }
        }

        for (var plength = beforePage; plength <= afterPage; plength++) {
            if (plength > totalPagesrefer) {
                continue;
            }
            if (plength == 0) { 
                plength = plength + 1;
            }
            if (referpage == plength) {
                active = "active";
            } else {
                active = "";
            }
            liTagRef += `<li class="numb ${active}" data-curitem="${plength}" onclick="createPaginationRefer(totalPagesrefer, ${plength})"><span>${plength}</span></li>`;
        }

        if (referpage < totalPagesrefer - 1) {
            if (referpage < totalPagesrefer - 2) {
                liTagRef += `<li class="dots"><span>...</span></li>`;
            }
            liTagRef += `<li class="last numb" data-curitem="${totalPagesrefer}" onclick="createPaginationRefer(totalPagesrefer, ${totalPagesrefer})"><span>${totalPagesrefer}</span></li>`;
        }

        if (referpage < totalPagesrefer) { 
            liTagRef += `<li class="btn next" onclick="createPaginationRefer(totalPagesrefer, ${referpage + 1})"><span>Next</span></li>`;
        }
        refer.innerHTML = liTagRef;
        return liTagRef;
    }
}
}
...
<script>
    if ($('#thispageTablecountpendingforapproval').length > 0) {
        const pendingforapproval = document.querySelector("#ajexpaginationpendingforapproval ul");
        $("#ajexpaginationpendingforapproval").hide();
        var totalrowspendingforapproval = parseInt($('#thispageTablecountpendingforapproval').html());
        console.log(totalrowspendingforapproval + ' pendfor app-savan111');
        var totalPagespendingforapproval = Math.ceil(totalrowspendingforapproval / 10);
        if (totalPagespendingforapproval > 1) {
            $("#ajexpaginationpendingforapproval").show();
            let pendingforapprovalpage = 1;
            pendingforapproval.innerHTML = createPaginationsforapp(totalPagespendingforapproval, pendingforapprovalpage);
        }

        function createPaginationsforapp(totalPagespendingforapproval, pendingforapprovalpage) {
            let liTagsfa = '';
            let active;
            let beforePage = pendingforapprovalpage - 1;
            let afterPage = pendingforapprovalpage + 1;
            loadActionDataPendingforApproval(pendingforapprovalpage);
            
            if (pendingforapprovalpage > 1) {
                liTagsfa += `<li class="btn prev" onclick="createPaginationsforapp(${totalPagespendingforapproval}, ${pendingforapprovalpage - 1})"><span>Prev</span></li>`;
            }

            if (pendingforapprovalpage > 2) {
                liTagsfa += `<li class="first numb" data-curitem="1" onclick="createPaginationsforapp(${totalPagespendingforapproval}, 1)"><span>1</span></li>`;
                if (pendingforapprovalpage > 3) {
                    liTagsfa += `<li class="dots"><span>...</span></li>`;
                }
            }

            for (var plength = beforePage; plength <= afterPage; plength++) {
                if (plength > totalPagespendingforapproval) {
                    continue;
                }
                if (plength == 0) {
                    plength = plength + 1;
                }
                if (pendingforapprovalpage == plength) {
                    active = "active";
                } else {
                    active = "";
                }
                liTagsfa += `<li class="numb ${active}"  data-curitem="${plength}" onclick="createPaginationsforapp(${totalPagespendingforapproval}, ${plength})"><span>${plength}</span></li>`;
            }

            if (pendingforapprovalpage < totalPagespendingforapproval - 1) {
                if (pendingforapprovalpage < totalPagespendingforapproval - 2) {
                    liTagsfa += `<li class="dots"><span>...</span></li>`;
                }
                liTagsfa += `<li class="last numb" data-curitem="${totalPagespendingforapproval}" onclick="createPaginationsforapp(${totalPagespendingforapproval}, ${totalPagespendingforapproval})"><span>${totalPagespendingforapproval}</span></li>`;
            }

            if (pendingforapprovalpage < totalPagespendingforapproval) {
                liTagsfa += `<li class="btn next" onclick="createPaginationsforapp(${totalPagespendingforapproval}, ${pendingforapprovalpage + 1})"><span>Next</span></li>`;
            }

            pendingforapproval.innerHTML = liTagsfa;
            return liTagsfa;
        }
    }
...
<script>
    ...
    if($('#verified_view_cnt').html() != undefined || $('#verified_view_cnt').html() != null){
    const pendingforapprovalvers = document.querySelector("#verifiedDataLoader ul");
    $("#verifiedDataLoader").hide();
    var totalrowspendingforapprovalvers = $('#verified_view_cnt').html();
    var totalPagespendingforapprovalvers = Math.ceil(totalrowspendingforapprovalvers / 10);
    
    if(totalPagespendingforapprovalvers >1){
        $("#verifiedDataLoader").show();
    let pendingforapprovalpagevers = 1;

    pendingforapprovalvers.innerHTML = createPaginationsforapps(totalPagespendingforapprovalvers, pendingforapprovalpagevers);
 
    function createPaginationsforapps(totalPagespendingforapprovalvers, pendingforapprovalpagevers) {
        let liTagsfave = '';
        let active;
        let beforePage = pendingforapprovalpagevers - 1;
        let afterPage = pendingforapprovalpagevers + 1;
        loadVerifiedData(pendingforapprovalpagevers);
        if (pendingforapprovalpagevers > 1) {
            liTagsfave += `<li class="btn prev" onclick="createPaginationsforapp(totalPagespendingforapprovalvers, ${pendingforapprovalpagevers - 1})"><span>Prev</span></li>`;
        }

        if (pendingforapprovalpagevers > 2) {
            liTagsfave += `<li class="first numb" onclick="createPaginationsforapps(totalPagespendingforapprovalvers, 1)"><span>1</span></li>`;
            if (pendingforapprovalpagevers > 3) {
                liTagsfave += `<li class="dots"><span>...</span></li>`;
            }
        }

        for (var plength = beforePage; plength <= afterPage; plength++) {
            if (plength > totalPagespendingforapprovalvers) {
                continue;
            }
            if (plength == 0) { 
                plength = plength + 1;
            }
            if (pendingforapprovalpagevers == plength) {
                active = "active";
            } else {
                active = "";
            }
            liTagsfave += `<li class="numb ${active}" onclick="createPaginationsforapps(totalPagespendingforapprovalvers, ${plength})"><span>${plength}</span></li>`;
        }

        if (pendingforapprovalpagevers < totalPagespendingforapprovalvers - 1) {
            if (pendingforapprovalpagevers < totalPagespendingforapprovalvers - 2) {
                liTagsfave += `<li class="dots"><span>...</span></li>`;
            }
            // liTagsfave += `<li class="last numb" onclick="createPaginationsforapp(totalPagespendingforapprovalver, ${pendingforapprovalpagever})"><span>${pendingforapprovalpagever}</span></li>`;
            liTagsfave += `<li class="last numb" onclick="createPaginationsforapps(totalPagespendingforapprovalvers, ${totalPagespendingforapprovalvers})"><span>${totalPagespendingforapprovalvers}</span></li>`;
        }

        if (pendingforapprovalpagevers < totalPagespendingforapprovalvers) { 
            liTagsfave += `<li class="btn next" onclick="createPaginationsforapps(totalPagespendingforapprovalvers, ${pendingforapprovalpagevers + 1})"><span>Next</span></li>`;
        }
        pendingforapprovalvers.innerHTML = liTagsfave;
        return liTagsfave;
    }
}
}
