...
<input type="hidden" value="<?= !empty($userDetails->region) ? $userDetails->region : '' ?>" name="uregion"
    class="uregion" />
<input type="hidden" value="<?= !empty($userDetails->sub_region) ? $userDetails->sub_region : '' ?>" name="usub_region"
    class="usub_region" />
<input type="hidden" value="<?= !empty($userDetails->staffid) ? $userDetails->staffid : '' ?>" name="ustaffid"
    class="ustaffid" />
<input type="hidden" class="dashboard" name="dashboard" value="dashboard" />
<input type="hidden" class="totalTabelrows" value="<?php echo  (!empty($action_items_cnt)) ? $action_items_cnt : 0 ?> ">
<div id="wrapper">
    <div class="content">
        <div class="new-dashboard">
            ...
            <div class="summary-section">
                ...
                if (!in_array($userRole, ['qc'])) { ?>
                    ...
                    <div class="submitForApproval-bg" onclick="reportFilter(12)">
                        <!-- <figure>
                            <img src="<?php echo base_url('assets/images/esc.png') ?>" alt="">
                        </figure> -->
                        <div>
                            <label class="submitforapproval_total">
                                <?php echo $submitforapproval; ?>
                            </label>
                            <span><?php if($userRole=='ata'){ echo _l('total_submitforapp'); }else{ echo _l('total_pendingforapp');} ?></span>
                        </div>
                        <span class="icon-circle">
                ...
                <?php } ?>
                    ...
                    <?php if($userRole=='aa'){ ?>
                    <div class="unassigned-bg" onclick="reportFilter(9)">
                        <!-- <figure>
                            <img src="<?php echo base_url('assets/images/wip.png') ?>" alt="">
                        </figure> -->
                        <div>
                            <label class="unassigned_total">
                                <?php echo $unassigned; ?>
                            </label>
                            <span class=""><?php echo _l('total_unassigned'); ?></span>
                        </div>
                        <span class="icon-circle">
                            ...
                            <svg viewBox="0 0 50 50">
                                <g id="Group_2761" data-name="Group 2761" transform="translate(-1033 -266)">
                                    <circle id="Ellipse_3" data-name="Ellipse 3" cx="25" cy="25" r="25"
                                        transform="translate(1033 266)" fill="#fff" />
                                    <g id="Group_2738" data-name="Group 2738" transform="translate(602.982 827.329)">
                                        <path id="Path_21" data-name="Path 21"
                                            d="M686.1-334.005c-.11.453-.183.918-.335,1.355a5.834,5.834,0,0,1-6.134,3.915,5.867,5.867,0,0,1-5.173-4.816,5.854,5.854,0,0,1,4.924-6.813,5.849,5.849,0,0,1,6.662,5.022,1.4,1.4,0,0,0,.056.2Zm-3.831-3.587-2.1,2.113-2.035-2.047-.85.881,2.056,2.053-2.061,2.052.949.965,2.076-2.093,2,2.016.938-.912-2.095-2.042,2.042-2.042Z"
                                            transform="translate(-214.06 -193.217)" fill="#c61a3b" />
                                        <path id="Path_22" data-name="Path 22"
                                            d="M459.83-371.522h-18.4a13.585,13.585,0,0,1,6.161-12.737,8.845,8.845,0,0,0,7.277,3.7,8.849,8.849,0,0,0,7.276-3.7,13.366,13.366,0,0,1,2.766,2.379,7.048,7.048,0,0,0-5.153,3.918A7.1,7.1,0,0,0,459.83-371.522Z"
                                            transform="translate(0 -152.944)" fill="#c61a3b" />
                                        <path id="Path_23" data-name="Path 23"
                                            d="M535.554-550.727a6.37,6.37,0,0,1,6.353,6.361A6.373,6.373,0,0,1,535.529-538a6.373,6.373,0,0,1-6.339-6.376A6.372,6.372,0,0,1,535.554-550.727Z"
                                            transform="translate(-80.68)" fill="#c61a3b" />
                                    </g>
                                </g>
                            </svg>
            ...
            <input type="hidden" name="action_item_list" class="action_item_list"
                value="<?php echo ACTION_ITEM_LIST; ?>">
            <?php
            $userRole = $GLOBALS['current_user']->role_slug_url;
            $activated ='';
            if( $userRole == 'ar'){
            ...
            if (in_array($userRole, [ 'ata', 'aa','qc','at'])) { ?>
                <h2 class="action-head"><?php echo _l('total_your_actio_item');?></h2>
                <ul class="nav nav-tabs dashboard-tab">
                    
                    <?php $actionView = ($userRole=='aa')?_l('total_unassigned'):_l('total_delayed');
                    $upcomingView = ($userRole=='aa')?_l('total_verified'):'Upcoming Deadlines';?>

                    <?php if($userRole != 'qc' && $userRole != 'ar'){
                        $viewedClass = ''; 
                        if($userRole == 'aa'){
                            $viewedClass = 'style=display:none;';
                        }
                        ?>

                        <li class="active action_items_data"><a href="#action-items" data-toggle="tab"><?php echo $actionView;?> (<span
                        class="action_items_cnt1" <?php echo $viewedClass;?>><?php echo (!empty($action_items_notview)) ? $action_items_notview : 0; ?>/</span><span id="thispageTablecount"><?php echo (!empty($action_items_cnt)) ? $action_items_cnt : 0; ?></span>)</a></li>

                     <?php } 
                     
                    
                            if($userRole=='aa' || $userRole=='qc'){?>
                    <!-- <li class="upcoming_deadline_data"><a href="#upcoming-deadlines" data-toggle="tab">
                    <?php //echo $upcomingView;?> (<span class="upcoming_deadline_cnt">
                                <?php //echo (!empty($upcoming_deadline_data_cnt)) ? $upcoming_deadline_data_cnt : 0; ?>
                            </span>)</a></li> -->
                            ...
                            <?php if($userRole=='qc') { 
                                $actionView = _l('total_closed');
                            ?>

                            <li class="active action_items_data"><a href="#action-items" data-toggle="tab"><?php echo $actionView;?> (<span
                                        class="action_items_cnt1"><?php echo (!empty($action_items_notview_closed)) ? $action_items_notview_closed : 0; ?>/</span><span id="thispageTablecount"><?php echo (!empty($action_items_closed_cnt)) ? $action_items_closed_cnt : 0; ?></span>)</a></li>
                            <?php } ?>

                            <?php $upcomingView = _l('total_verified'); ?>
                            
                            <li class="verified_data"><a href="#verified-deadlines" data-toggle="tab" style="display:flex;align-items:center;"> <?php echo $upcomingView;?><span style="margin-left:5px;">(<span class="verified_cnt"><?php echo (!empty($action_items_notview_verified)) ? $action_items_notview_verified : 0; ?></span>/<span id="verified_view_cnt"><?php echo (!empty($action_items_verified_cnt)) ? $action_items_verified_cnt : 0; ?></span>)</span></a>
                            </li>

                            <?php } ?>
                          <?php if (in_array($userRole, ['at'])) { ?>
                            ...
                            <?php if (in_array($userRole, ['at'])) { ?>
                                <li class="<?php echo $activated;?> action_items_data_refered"><a href="#action-refered" data-toggle="tab"><?php echo _l('total_referred');?> (<span
                                class="action_items_refered_cnt"><?= (!empty($action_items_notview_refered)) ? $action_items_notview_refered : 0; ?>/</span><span id="thispageTablecountrefer"><?= (!empty($action_items_refered_cnt)) ? $action_items_refered_cnt : 0; ?></span>)</a></li>
                                <li class="action_items_data_pending_for_approval"><a href="#action-pending-for-approval" data-toggle="tab"><?php if($userRole=='ata'){ echo _l('total_submitforapp'); }else{ echo _l('total_pendingforapp');} ?> (<span
                                class="action_items_pendingforapproval_cnt"><?php echo (!empty($action_items_notview_sendingforapproval)) ? $action_items_notview_sendingforapproval : 0; ?>/</span><span id="thispageTablecountpendingforapproval"><?php echo (!empty($action_items_pendingforapproval_cnt)) ? $action_items_pendingforapproval_cnt : 0; ?></span>)</a></li>
                            <?php }?>
                            <?php }?>  
                </ul>
                ...
                <?php if( $userRole != 'ar'){ ?>
                    <div class="tab-pane fade active in" id="action-items">
                        ...
                        <div class="d-block text-center load-btn-action-item  hide">
                    ...
                    <?php } ?>
                    <?php if($userRole=='aa' || $userRole=='qc'){?>
                    <div class="tab-pane fade" id="verified-deadlines">
                        <div class="dashboard-table verified-data">
                            <div id="verifiedItems" class="table-row-group"></div>
                        </div>
                        <input type="hidden" id="nextpageno" value="0">
                        <div class="d-block text-center load-btn load-btn-verified hide">
                            <div class="pagination" id="verifiedDataLoader">
                                <ul></ul>
                            </div>
                        </div>
                        <!-- <div class="load-btn load-btn-verified hide">
                            <button class="btn loadMore-btn" id="verifiedDataLoader">Show More</button>
                        </div> -->
                        <div class="dashboard-table no-verified-data hide">
                            <p class="text-center no-data-row"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i>
                                You have no verified tickets.</p>
                        </div>
                    </div>
                    <?php }
                    ...
                    <div class="tab-pane fade in <?php echo $activated;?>" id="action-refered">
                        <div class="dashboard-table action-items-data-refered">
                            
                            <div id="actionItemsrefered" class="table-row-group"></div>
                        </div>
                        <input type="hidden" id="pageno" value="0">
                        <div class="d-block text-center load-btn-action-item-refered hide">
                    ...
                    <div class="tab-pane fade in" id="action-pending-for-approval">
                        <div class="dashboard-table action-items-data-pending-for-approval">
                            
                            <div id="actionItemspendingforapproval" class="table-row-group"></div>
                        </div>
                        <input type="hidden" id="pageno" value="0">
                        <div class="d-block text-center load-btn-action-item_pendingforapproval hide">
                            <div class="pagination" id="ajexpaginationpendingforapproval">
                                <ul></ul>
                            </div>
                        </div>
                        <div class="dashboard-table load-btn-action-item_pendingforapproval hide">
                            <p class="text-center no-data-row"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i>
                                You have no action items.</p>
                        </div>
                    </div>
