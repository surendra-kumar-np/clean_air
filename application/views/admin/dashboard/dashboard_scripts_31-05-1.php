...
<script>
    ...
    if($('#thispageTablecountpendingforapproval').html() != undefined || $('#thispageTablecountpendingforapproval').html() != null){
    const pendingforapproval = document.querySelector("#ajexpaginationpendingforapproval ul");
    $("#ajexpaginationpendingforapproval").hide();
    var totalrowspendingforapproval = $('#thispageTablecountpendingforapproval').html();
    ...
    function createPaginationsforapp(totalPagespendingforapproval, pendingforapprovalpage) {
        ...
        if (pendingforapprovalpage < totalPagespendingforapproval - 1) {
            ...
            liTagsfa += `<li class="last numb" onclick="createPaginationsforapp(totalPagespendingforapproval, ${totalPagesrefer})"><span>${totalPagesrefer}</span></li>`;
        }
...
<script>
    ...
    const pendingforapprovalver = document.querySelector("#verifiedDataLoader ul");
    $("#verifiedDataLoader").hide();
    var totalrowspendingforapprovalver = $('#verified_view_cnt').html();
    var totalPagespendingforapprovalver = Math.ceil(totalrowspendingforapprovalver / 10);
    
    if(totalPagespendingforapprovalver >1){
        $("#verifiedDataLoader").show();
    let pendingforapprovalpagever = 1;

    pendingforapprovalver.innerHTML = createPaginationsforapp(totalPagespendingforapprovalver, pendingforapprovalpagever);
 
    function createPaginationsforapp(totalPagespendingforapprovalver, pendingforapprovalpagever) {
        let liTagsfave = '';
        let active;
        let beforePage = pendingforapprovalpagever - 1;
        let afterPage = pendingforapprovalpagever + 1;
        loadVerifiedData(pendingforapprovalpagever);
        if (pendingforapprovalpagever > 1) {
            liTagsfave += `<li class="btn prev" onclick="createPaginationsforapp(totalPagespendingforapprovalver, ${pendingforapprovalpagever - 1})"><span>Prev</span></li>`;
        }

        if (pendingforapprovalpagever > 2) {
            liTagsfave += `<li class="first numb" onclick="createPaginationsforapp(totalPagespendingforapprovalver, 1)"><span>1</span></li>`;
            if (pendingforapprovalpagever > 3) {
                liTagsfave += `<li class="dots"><span>...</span></li>`;
            }
        }

        for (var plength = beforePage; plength <= afterPage; plength++) {
            if (plength > totalPagespendingforapprovalver) {
                continue;
            }
            if (plength == 0) { 
                plength = plength + 1;
            }
            if (pendingforapprovalpagever == plength) {
                active = "active";
            } else {
                active = "";
            }
            liTagsfave += `<li class="numb ${active}" onclick="createPaginationsforapp(totalPagespendingforapprovalver, ${plength})"><span>${plength}</span></li>`;
        }

        if (pendingforapprovalpagever < totalPagespendingforapprovalver - 1) {
            if (pendingforapprovalpagever < totalPagespendingforapprovalver - 2) {
                liTagsfave += `<li class="dots"><span>...</span></li>`;
            }
            ...
            liTagsfave += `<li class="last numb" onclick="createPaginationsforapp(totalPagespendingforapprovalver, ${totalPagespendingforapprovalver})"><span>${totalPagespendingforapprovalver}</span></li>`;
        }

        if (pendingforapprovalpagever < totalPagespendingforapprovalver) { 
            liTagsfave += `<li class="btn next" onclick="createPaginationsforapp(totalPagespendingforapprovalver, ${pendingforapprovalpagever + 1})"><span>Next</span></li>`;
        }
        pendingforapprovalver.innerHTML = liTagsfave;
        return liTagsfave;
    }
}
}
