						...
						<div class="panel-header">
							<?php if (has_permission($permission, '', 'create')) { ?>
								<a href="#" onclick="new_staff(); return false;" class="btn btn-custom add-area-admin pull-right display-block">
									<?php echo _l('add_reviewer');?>
								</a>
							<?php } ?>
							<h1><?php echo _l('manage_reviewers'); ?> <span><?php echo _l('here_you_can_view_add_edit_and_deactivate_reviewers'); ?> </span></h1>
			...
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title">
						<span class="edit-title"><?php echo _l('edit_reviewer');?></span>
						<span class="add-title"><?php echo _l('add_reviewer');?></span>
					</h4>
				</div>
				
				<div class="modal-body">
					<div id="addition"></div>
					<p class="form-instruction add-title"><?php echo _l('fill_in_the_following_fields_to_add_a_reviewer');?></p>
					<p class="form-instruction edit-title"><?php echo _l('fill_in_the_following_field(s)_to_edit_a_reviewer');?></p>
...
<script>
	$(function() {
		...
		initDataTable('.table-staff-ar', window.location.href, [3], [6], undefined, [0, 'asc'], '',columnDefs);
		...
		$.ajax({
			...
			success: function(response) {
				if (response.success == true) {
					...
					$('#add_area_admin input[name="designation"]').val(response.staff.designation);
