						...
						<div class="panel-header">
							<?php if (has_permission($permission, '', 'create')) { ?>
								<a href="#" onclick="new_staff(); return false;" class="btn btn-custom add-area-admin pull-right display-block">
									<?php echo _l('add_project_leader'); ?>
								</a>
							<?php } ?>
							<h1><?php echo _l('manage_project_leaders');?> <span><?php echo _l('here_you_can_view_add_edit_and_deactivate_project_leaders');?> </span></h1>
		...
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">
					...
					<span class="edit-title"><?php echo _l('edit_project_leader');?></span>
					<span class="add-title"><?php echo _l('add_project_leader');?></span>
				</h4>
			</div>
			<div class="modal-body">
				<div id="additional"></div>
				<p class="form-instruction add-title"><?php echo _l('fill_in_the_following_fields_to_add_a_project_leader');?></p>
				<p class="form-instruction edit-title"><?php echo _l('fill_in_the_following_field(s)_to_edit_a_project_leader');?></p>
				...
				<div class="form-group" app-field-wrapper="region_id">
					<div class="form-select-field">
						<select name="region" class="selectpicker show-tick" id="region_id" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" data-live-search="true" title="<?php echo _l('select_city_corporation_req');?>">
						</select>
				...
				<div class="form-group" app-field-wrapper="sub_region_id">
					<div class="form-select-field">
						<select name="sub_region" class="form-control selectpicker show-tick" id="sub_region_id" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" data-live-search="true" title="<?php echo _l('select_municipal_zone_req')?>">
						</select>
				...
				<div class="form-group" app-field-wrapper="reviewer_id">
					<div class="form-select-field">
						<select name="reviewer" class="form-control selectpicker show-tick" id="reviewer_id" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" data-live-search="true" title="<?php echo _l('select_reviewer_req');?>">
						</select>
						<label class="select-label"><?php echo _l('reviewer');?><span class="required_red">*</span></label>
						<p id="reviewer_id-error" class="text-danger required_size"></p>
					</div>
				</div>

				<!-- <div class="form-group" app-field-wrapper="categories">
					<div class="form-select-field">
						<select name="categories[]" class="form-control selectpicker show-tick" id="categories" data-width="100%" data-none-selected-text="<?php //echo _l('dropdown_non_selected_tex'); ?>" data-live-search="true" data-actions-box="true" multiple title="<?php //echo _l('select_action_items_req');?>">
							<?php //foreach ($categories as $key => $cat_name) { ?>
								<option value="<?php //echo $cat_name['id'] ?>"><?php //echo $cat_name['issue_name'] ?></option>
							<?php //} ?>
						</select>
						<label class="select-label"><?php //echo _l('categories_at_req'); ?></label>
						<p id="categories-error" class="text-danger required_size"></p>
					</div>
				</div> -->
				<?php
				echo "<div class='form-group'>" . render_input('firstname', _l('at_full_name_req'), '', 'text', ['required' => 'required', 'autocomplete' => 'off', 'id' => 'admin_name']) . "</div>";
				...
				<?php
				echo "<div class='form-group'>" . render_input('designation', _l('designation_req'), '', 'designation', ['required' => 'required', 'autocomplete' => 'off', 'id' => 'designation']) . "</div>";
				echo "<div class='form-group'>" . render_input('email', _l('email_req'), '', 'email', ['required' => 'required', 'autocomplete' => 'off', 'id' => 'admin_email']) . "</div>";
				echo "<div class='form-group'>" . render_input('phonenumber', _l('phone_req'), '', 'text', ['required' => 'required', 'autocomplete' => 'off', 'id' => 'admin_phone', 'onkeypress' => 'return isNumberKey(event)', 'maxlength' => '10']) . "</div>";
...
<script>
	...
	$(function() {
		...
		initDataTable('.table-staff-at', window.location.href, [8], [8], undefined, [0, 'asc'], '', columnDefs);
	...
	function edit_admin(invoker, id,orgId) {
		...
		getReviewers(regionId, $(invoker).data('rwid'));
		window.reviewerToSelect = $(invoker).data('rwid');
		window.orgToSelect = orgId;
		window.staffId =id;

		//getOrgDept(id,orgId,regionId);

		// alert($(invoker).data('designation'));
		// alert($(invoker).data('phone'));
		...
		// 	if($('#organization_new').val()!== null || $('#organization_new').val()==''){
		// 		console.log("organizationId-329",$('#organization_new').val());
		// 		$('#organization_new').prop('disabled', true).selectpicker('refresh');
		// 	}else{
		// 		console.log("organizationId-332",$('#organization_new').val());
		// 		$('#organization_new').prop('disabled', false).selectpicker('refresh');
		// 	}
		// }, 5000);
		triggerInputClass();
	...
	$(document).on('changed.bs.select', '#region_id', function(e) {
		...
		getReviewers($(e.currentTarget).val(), window.reviewerToSelect);
		if(window.reviewerToSelect){
			window.reviewerToSelect = null;
		}
	...
	const getReviewers = (city_id, rewiewerId = null) => {
		let area = "<?= $area ?>";
		let data = {
			'area_id': area,
			'region_id': city_id
		}
		$.post(admin_url + 'staff/get_city_reviewers', data).done((res) => {
			res = JSON.parse(res);
			if (res.success == true) {
				let options = "";
	...
	// 		console.log(selectedCats,'here cate');
