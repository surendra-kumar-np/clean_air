...
$ward_name  = '';
if(!empty($GLOBALS['current_user']->ward_name)){
    $ward_name = "(".$GLOBALS['current_user']->ward_name.")";
}

$zone = $GLOBALS['current_user']->location;
$subregion_name = $zone['subregion_name'] ? ','.$zone['subregion_name'] : '';
$organisation = $GLOBALS['current_user']->organisation?','.$GLOBALS['current_user']->organisation:'';
$designation = $GLOBALS['current_user']->designation?','.$GLOBALS['current_user']->designation:'';
?>
<div id="header">
    ...
    <div class="profile-info">
        <a href="<?php echo admin_url('staff/edit_profile') ?>">
            ...
            <span><?php echo trim($GLOBALS['current_user']->full_name); ?> (<?php echo $GLOBALS['current_user']->role_name; ?>)<?php echo $designation; ?><?php echo $subregion_name; ?> <?php echo $organisation; ?> <?php //echo $ward_name; ?></span>
            <!-- <a href="<?php echo 'mailto:' . $GLOBALS['current_user']->email; ?>"><?php echo $GLOBALS['current_user']->email; ?></a> -->
        </a>
        <?php /* if($GLOBALS['current_user']->area!=0){
    ...
    <!-- <li class="dropdown-submenu pull-right customers-nav-item-languages">
    ...
    <li class="dropdown-submenu pull-right customers-nav-item-languages">
                <a href="#" tabindex="-1">
                <?php if(get_staff_default_language()==''){ echo ucfirst($activeLanguage[0]);}else{ echo ucfirst(get_staff_default_language());} ?>
                <?php //echo _l('system_default_string'); ?>
                </a>
                <ul class="dropdown-menu dropdown-menu-left language-dropdown">
                    <!-- <li class="<?php //if(get_staff_default_language() == ""){echo 'active';} ?>">
                        <a href="<?php //echo site_url('admin/staff/change_language'); ?>">
                        <?php //echo _l('system_default_string'); ?>
                        </a>
                    </li> -->
                    <?php foreach($this->app->get_available_languages() as $user_lang) { ?>
                    <?php
                    if (in_array($user_lang, $activeLanguage))
                    {?>

                    <li <?php //if(get_staff_default_language() == $user_lang){echo 'class="active"';} ?>>
                        <a href="<?php echo site_url('admin/staff/change_language/'.$user_lang); ?>">
