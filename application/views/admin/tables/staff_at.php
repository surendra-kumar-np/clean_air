...
$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['GROUP_CONCAT(issue_name SEPARATOR ", ") as categories', 'GROUP_CONCAT(issue_id SEPARATOR ",") as issue_id','org_id','designation','staffid', 'area', 'roles.slug_url', 'region.id as region_id', 'sub_region.id as sub_region_id'], 'GROUP BY staffid');
// print_r($result);
// die;
...
foreach ($rResult as $aRow) {
    ...
    $options = icon_btn('', 'pencil-square-o', 'btn-default', [
        ...
        'onclick' => 'edit_admin(this,' . $aRow['staffid'] . ','.$aRow['org_id'].'); return false', 'data-name' => $aRow['staff.firstname'], 'data-email' => $aRow['staff.email'], 'data-status' => $aRow['staff.active'], 'data-department' => $aRow['area'], 'data-staffid' => $aRow['staffid'], 'data-phone' => $aRow['staff.phonenumber'], 'organization.name' => $aRow['organization.name'], 'data-rid' => $aRow['region_id'], 'data-srid' => $aRow['sub_region_id'], 'data-rwid' => $aRow['staff_assistance.assistant_id'], 'data-issue-ids' => $aRow['issue_id'],'data-designation' => $aRow['designation'] 
    ]);
    $row[] = $options;
