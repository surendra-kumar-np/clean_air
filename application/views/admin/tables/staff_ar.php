...
$join         = [' LEFT JOIN ' . db_prefix() . 'roles ON ' . db_prefix() . 'roles.roleid = staff.role LEFT JOIN ' . db_prefix() . 'organization ON ' . db_prefix() . 'organization.id = staff.org_id LEFT JOIN ' . db_prefix() . 'staff_region ON ' . db_prefix() . 'staff_region.staff_id = ' . db_prefix() . 'staff.staffid LEFT JOIN ' . db_prefix() . 'region ON ' . db_prefix() . 'region.id = ' . db_prefix() . 'staff_region.region '];


$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['staffid', 'area','org_id', 'roles.slug_url','phonenumber','region.id as region_id']);
...
foreach ($rResult as $aRow) {
    ...
    $options = icon_btn('staff/edit_profile/' . $aRow['staffid'], 'pencil-square-o', 'btn-default', [
        ...
        'onclick' => 'edit_admin(this,' . $aRow['staffid'] . ','.$aRow['org_id'].'); return false', 'data-name' => $aRow['staff.firstname'], 'data-email' => $aRow['staff.email'], 'data-status' => $aRow['staff.active'], 'data-department' => $aRow['area'], 'data-staffid' => $aRow['staffid'], 'data-phone' => $aRow['staff.phonenumber'], 'organization.name' => $aRow['organization.name'], 'data-rid' => $aRow['region_id']
    ]);
    $row[] = $options;
