...
$where        = ['AND '.db_prefix().'issue_milestones.is_active = 1','AND '.db_prefix().'issue_categories.is_deleted = 0','AND '.db_prefix().'issue_milestones.area_id = '.get_staff_area_id(get_staff_user_id()).''];
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'issue_name') {
         ...
         if ($aColumns[$i] == 'region_id') {
            $_data = get_region_name($aRow['region_id']);
         }
         
         if($aColumns[$i] == 'organization_id'){
                $_data = get_organization_name($aRow['organization_id']);
         }
         if ($aColumns[$i] == 'reminder_one') {
    ...
    if(in_array($aRow['id'],$temp)==false){
        ...
        if (has_permission('categories_manage', '', 'edit')) {
            ...
            'onclick' => 'edit_issue(this,' . $aRow['id'] . '); return false" data-name="' . $aRow['issue_name'] .
            '" data-region_id="' . get_issue_field_data($aRow['id'],'region_id') . 
            '" data-organization_id="' . get_issue_field_data($aRow['id'],'organization_id') . 
            '" data-milestoneid="' . get_milestone_of_issue($aRow['id'],'id') . 
            '"data-parent="'. get_issue_field_data($aRow['id'],'parent_issue_id') .'"data-reminder_one="' . get_milestone_of_issue($aRow['id'],'reminder_one') .'"data-reminder_two="' . get_milestone_of_issue($aRow['id'],'reminder_two') .'"data-milestone="' . get_milestone_of_issue($aRow['id'],'milestone_name') . '"data-duration="' . get_milestone_of_issue($aRow['id'],'days')  . '"'
            ]);
        }
        // else{
        //     if(!check($aRow['id'])){
