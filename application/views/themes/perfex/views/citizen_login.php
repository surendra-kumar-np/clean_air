...
<script>
    ...
    $(document).ready(function(){
	    ...
	    $('.captcha-refresh').on('click', function(){
			$.get('<?php echo site_url(); ?>citizensauthentication/refreshcaptcha', function(data){
        ...
        $(document).ready(function() {
			...
			$(document).on('click', '.openotp', function(){
                ...
                if (validateMobileNumber(mobileNumber)) {
                    $.ajax({
                        url: '<?php echo site_url(); ?>/citizensauthentication/generateotp', 
                        type: 'POST',
                        ...
                        success: function(response) {
							...
							if (data.status === 'success') {
								...
								// Action on failure
								$("#mobile_hints").show();
								$("#mobile_hints").html('OTP could not be sent.Due to some technical issues.');
							}
			...
			$(document).on('click', '#auth_login_otp_button', function(){
                ...
                if (validateOTPNumber(otpNumber)) {
                    $.ajax({
                        url: '<?php echo site_url(); ?>/citizensauthentication/otplogin', 
                        type: 'POST',
                        data: { otp: otpNumber },
                        success: function(response) {
                            // alert(response);
							var data = JSON.parse(response);
							if (data.status === 'success') {
								window.location.href = '<?php echo site_url(); ?>/clients/open_ticket';
							} else {
								$('.phonebox').addClass('d-none');
				                $('.verifyotpbox').removeClass('d-none');
								// Action on failure
