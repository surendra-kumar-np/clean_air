    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="panel_s p-0">
				...
				<?php echo form_open_multipart('citizens/open_ticket',array('id'=>'open-new-ticket-form')); ?>
				
                <div class="panel-body">
                    <div class="open-ticket-subject">
                        <div class="panel-header">
                            <h1>
								<?php echo _l('Raise a new Complaint'); ?>
								<span>Here you can raise a new complaint in your City/ Corporation. </span>
							</h1>
						...
						<div class="row m-0">
							...
							<!-- <div class="form-select-field singleSelect">
								...
								<div class="form-select-field">
									...
									<option value="">Select State</option>
										<?php foreach(get_all_areas() as $area){ ?>
						...
						<div class="col-md-12 mT10">
							<div class="">
								<div class="form-group open-ticket-message-group">
									...
									<textarea type="text" id="message" name="message" class="form-textarea" rows="15"><?php echo set_value('message'); ?></textarea>
								...
								<label for="attachment" class="control-label"><?php echo _l('clients_ticket_attachments'); ?><span style="font-size:12px;">&nbsp;(Select Images and PDF only | Max size: 5 MB)</span></label>
								...
								<div class="form-group file-group row">
									<div class="col-lg-3">
										<div class="image-upload">
											...
											<input type="file" accept="application/pdf, image/jpeg, image/jpg, image/png, image/x-png" name="file[]" id="file1" class="form-control evidence-uploader" capture="camera" />
									...
									<div class="col-lg-3">
										<div class="image-upload">
											...
											<input type="file" accept="application/pdf, image/jpeg, image/jpg, image/png, image/x-png" name="file[]" id="file2" class="form-control evidence-uploader" capture="camera" />
											<!-- <span class="img-label">Upload Evidence</span> -->
										</div>
									...
									<div class="col-lg-3">
										<div class="image-upload">
											...
											<input type="file" accept="application/pdf, image/jpeg, image/jpg, image/png, image/x-png" name="file[]" id="file3" class="form-control evidence-uploader" capture="camera" />
											<!-- <span class="img-label">Upload Evidence</span> -->
									...
									<div class="col-lg-3">
										<div class="image-upload">
											...
											<input type="file" accept="application/pdf, image/jpeg, image/jpg, image/png, image/x-png" name="file[]" id="file4" class="form-control evidence-uploader" capture="camera" />
											<!-- <span class="img-label">Upload Evidence</span> -->
										</div>
							...
							<div class="mtop20 mB20">
								<button type="submit" class="btn btn-custom btn-submit-ticket" id="submitbutton"><?php echo _l('submit'); ?></button>
							</div>
						</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="modal-footer mT30 pL15">
						<input type="button" id="confirmsubmit" class="btn btn-custom btn-submit-ticket" value="Raise Complaint">
...
<script>
	...
	$(document).ready(function() {
		$("#area_id").val(4).change();
		
		area_selected();
		
		//droup-down disabled
		if($('#area_id').siblings().data("id") === 'area_id') {
			setTimeout(function(){
				$('#area_id').siblings().attr('data-toggle', '');
			}, 2000);
		}
	...
	$.ajax({
		type: "post",
		url: site_url + 'citizens/get_surveyor_detail' ,
		success: function (response) {
			const data=JSON.parse(response);
			if((data.area !=0 && data.area != null) && (data.region !=0 && data.region != null) && (data.subregion != 0 && data.subregion != null) ) {
	...
	$(".add_more_attachments_test").click(function(e) {
		 ...
		 <?php } ?>
			...
			<?php if(is_callcenter($this->session->userdata('client_user_id'))){ ?>
				...
				if(area !="" && region !="" && subregion != "" && landmark != "" && category !="" && message !="" && count==0 && name !="" && email!="" && phone !=""){
					...
					$.ajax({
						type: "post",
						url: site_url + 'citizens/check_geotagg_image',
						data:a,
						processData: false,
						contentType: false,
						success: function (response) {
		...
		const getRegionData = (areaid, val = '') => {
			...
			$.post(site_url + 'citizens/get_region', data).done((res) => {
				res = JSON.parse(res);
				...
				if (res.success == true) {
					...
					//option selected in mobile app
					if( isMobile.Android() ) {
						$('#region_id').val(1);//patna selected in mobile app
		...
		const getSubregion2 = (regionid, val = "") => {
			$.post(site_url + 'citizens/getsubregion', {
				"regionid": regionid
			}).done((res) => {
				res = JSON.parse(res);
				// console.log(res);
				...
				$.each(res.sub_region_list, function(indexInArray, value) {
					if (val != '' && val == value.id) {
						options +=`<option value='${value.id}' selected>${value.region_name}</option>`;
					} else {
						options +=`<option value='${value.id}'>${value.region_name}</option>`;
					}
				});

				$('#sub_region_id option').remove(); //remove old options
		...
		const getCategories = (areaid, val = '') => {
			...
			$.post(site_url + 'citizens/get_area_issues', data).done((res) => {
				res = JSON.parse(res);
				if (res.success == true) {
