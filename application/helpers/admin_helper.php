...
function get_organization_name($id)
{
    $CI          = &get_instance();
    $CI->db->select('name');
    $organization_id = json_decode($id);
    if(empty($id)){
        return '';
    }else
    if(is_array($organization_id)){
        $CI->db->where_in('id', $organization_id);
    }
   ...
   // $region_name = $rows[0];
    return rtrim($organization_name, ', ');
...
function get_region_name($id)
{
    ...
    $region_id = json_decode($id);
    if(empty($id)){
        return '';
    }else
    if(is_array($region_id)){
        $CI->db->where_in('id', $region_id);
    }
...
function getProjectAssignedUser2($projectId){
    $staff_id = 0;
    if (!empty($projectId)) {
        $CI = &get_instance();
...
function getProjectAssignedUser($projectId){
    ...
    if (!empty($projectId)) {
        $CI = &get_instance();
        // SELECT pm.staff_id, pm.assigned, pm.project_id, pa.id, pa.staff_id, pa.description_key, pa.status, s.role FROM project_members pm 
        // JOIN project_activity pa ON pa.project_id = pm.project_id 
        // JOIN staff s ON s.staffid = pm.staff_id AND  pa.project_id = 43653 
        // ORDER by pa.id DESC;
        $CI->db->select('pm.staff_id as pm_staff, pm.assigned, pm.project_id, pa.id, pa.staff_id, pa.description_key, pa.status, s.role' );
        $CI->db->join(db_prefix() . 'project_activity as pa', db_prefix() . 'pa.project_id = ' . db_prefix() . 'pm.project_id', 'left');
        $CI->db->join(db_prefix() . 'staff as s', db_prefix() . 's.staffid = ' . db_prefix() . 'pm.staff_id', 'left');
        $CI->db->where('pm.project_id', $projectId)
            ->where('pm.active', 1)->where('pm.assigned', 1)
            ->order_by('pa.id', 'DESC');
        $rows = $CI->db->get(db_prefix() . 'project_members as pm')->result();
        // pre($rows);
        // echo $CI->db->last_query();exit;
        if(!empty($rows)){
            // foreach($rows as $row){
            //     // echo "<pre>";print_r($row);echo "</pre>";
            //     if($row->description_key == 'ticket_assigned_to_ata' && $row->role==8 && $row->assigned==0){
            //         $staff_id = $rows[0]->pm_staff;
            //         break;
            //     }
            // }
            // if($staff_id==0){
            //     $staff_id = getProjectAssignedUser2($projectId);
            // }
            if($rows[0]->description_key == 'ticket_assigned_to_ata' && $rows[0]->role==8 && $rows[0]->assigned==0){
                $staff_id = $rows[0]->pm_staff;
            }
            else
            if(!empty($rows[1]) && $rows[1]->description_key == 'ticket_assigned_to_ata' && $rows[1]->role==8 && $rows[1]->assigned==0){
                $staff_id = $rows[1]->pm_staff;
            }else{
                $staff_id = getProjectAssignedUser2($projectId);
            }
        }
...
function getProjectActivityLongTerm($projectId){
    ...
    if (!empty($projectId)) {
        ...
        $CI->db->select('status')
            ->where('project_id', $projectId)
            ->where('status', 10);
        $rows = $CI->db->get(db_prefix() . 'project_activity')->row();
        if(!empty($rows)){
            $longterm = 'Yes';
        }
    }
...
function getProjectAssignedPreviousUser($projectId,$condition=null){
    $staff_id = 0;
    if (!empty($projectId)) {
        $CI = &get_instance();
        $CI->db->select('additional_data')
            ->where('project_id', $projectId);
            if($condition == "close"){
                $CI->db->where('status', 12);
            } else if($condition == "lt"){
                $CI->db->where('status', 11);
            }
        ...
        if(!empty($rows)){
            $adata = json_decode($rows->additional_data);
            $additional_data = $adata;
            $staff_id = $additional_data->assigned_by;
        }
    }
    return $staff_id;    
}
