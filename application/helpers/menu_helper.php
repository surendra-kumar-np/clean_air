...
function app_init_admin_sidebar_menu_items()
{
    ...
    if($role_slug == 'ae-global' || $role_slug == 'ae-area' || $role_slug == 'ar' || $role_slug == 'at' || $role_slug == 'qc' || $role_slug == 'ata' || $role_slug == 'aa'){
    ...
    if($role_slug == 'aa'){
        // $CI->app_menu->add_sidebar_menu_item('cityplan', [
        //     'name'     => _l('cityplan'), 
        //     'href'     => admin_url('cityplan'),
        //     'icon'     => 'fa fa-user-circle-o',
        //     'position' => 62,
        // ]);
        $CI->app_menu->add_sidebar_menu_item('dashboardchecklist', [
            'name'     => _l('checklist'),
            'href'     => admin_url('dashboard/checklist'),
            'position' => 1,
            'icon'     => 'fa fa-home',
        ]);

        $CI->app_menu->add_sidebar_menu_item('manage-geography', [
            'slug'     => 'geography',
            'name'     => _l('manage_geography'),
            'href'     => admin_url('#'),
            'position' => 2,
            'icon'     => 'fa fa-user',
        ]);

        $CI->app_menu->add_sidebar_children_item('manage-geography', [
            'name'     => _l('city_corporation'),
            'slug'     => 'region',
        ...
        $CI->app_menu->add_sidebar_children_item('manage-geography', [
            'slug'     => 'org',
            'name'     => _l('manage_organization'),
            'href'     => admin_url('staff/organization'),
            'position' => 2,
        ]);
        ...
        $CI->app_menu->add_sidebar_children_item('manage-geography', [
            'name'     => _l('manageward'),
            'slug'     => 'wm',
            'href'     => admin_url('manageward'),
            'position' => 3,
        ]);
        $CI->app_menu->add_sidebar_children_item('manage-geography', [
            'name'     => _l('municipal_zone'),
            'slug'     => 'subregion',
            'href'     => admin_url('subregion'),
            'position' => 4,
        ]);
        // $CI->app_menu->add_sidebar_children_item('manage-geography', [
        //     'name'     => _l('subdivision_name'),
        //     'slug'     => 'subdivision',
        //     'href'     => admin_url('subdivision'),
        //     'position' => 4,
        // ]);
        //   if (has_permission('staff_ata', '', 'view_own')) {
        //       $CI->app_menu->add_sidebar_children_item('manage-user', [
        //           'name'     => _l('project_support'),
        //           'slug'     => 'ata',
        //           'href'     => admin_url('staff?role=ata'),
        //           'position' => 5,
        //       ]);
        //   }
        // $CI->app_menu->add_sidebar_menu_item('wardmanage', [
        //     'slug'     => 'wm',
        //     'name'     => _l('manage_ward'),
        ...
        //     'position' => 36,
        // ]);
        // $CI->app_menu->add_sidebar_menu_item('unassigneditem', [
        //     'name'     => _l('unassigned_item'),
        //     'icon'     => 'fa fa-list',
        //     'href'     => admin_url('dashboard/unassigneditem'),
        //     'position' => 36,
        // ]);
        // $CI->app_menu->add_sidebar_menu_item('organization', [
        //     'slug'     => 'org',
        //     'name'     => _l('manage_organization'),
    ...
    if(( ($role_slug == 'qc' || $role_slug == 'at' || $role_slug == 'ar' || $role_slug == 'ae-area' || $role_slug == 'ata') && $file_url != '') || /*($role_slug == 'ae-global' && $file_url == '')|| ($role_slug == 'ap-sa' && $file_url == '') ||*/ ($role_slug == 'aa') ){
        ...
        if( ($role_slug == 'aa') &&  $file_url != '' ){   
            $CI->app_menu->add_sidebar_children_item('master_plan', [
                ...
                'name'     => _l('view_city_action_plan'),
                // 'href'     => $file_url,
                'href'     => admin_url('cityplan/view'),
                // 'target'     => '_blank',
                'position' => 26,
            ]);
        }
        if( ($role_slug == 'qc' || $role_slug == 'at' ||  $role_slug == 'ar' || $role_slug == 'ae-area' || $role_slug == 'ata' ) &&  $file_url != '' ){   
            $CI->app_menu->add_sidebar_menu_item('master_plan', [
                'slug'     => 'view_master_plan',
                'name'     => _l('city_action_plan'),
                // 'href'     => $file_url,
  ...
  if (has_permission('staff', '', 'view')) {
      ...
      if (has_permission('staff_qc', '', 'view_own')) {
        $CI->app_menu->add_sidebar_children_item('manage-user', [
            'name'     => _l('surveyor_qc'),
            'slug'     => 'qc',
            'href'     => admin_url('staff?role=qc'),
            'position' => 6,
        ]);
    }
    if (has_permission('staff_sv', '', 'view_own')) {
        $CI->app_menu->add_sidebar_children_item('manage-user', [
            'name'     => _l('surveyor'),
            'slug'     => 'sv',
            'href'     => admin_url('staff?role=sv'),
            'position' => 7,
        ]);
    }
    ...
    */
    ...
    if($role != 8){
        $CI->app_menu->add_sidebar_menu_item('atrreport', [
            'name'     => _l('atrreport'), 
            'href'     => admin_url('reportatr/atr_role_wise_report'),
            'icon'     => 'fa fa-user-circle-o',
            'position' => 62,
        ]);
    }
