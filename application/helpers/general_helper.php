...
?>
    <script>
        ...
        function csrf_jquery_ajax_setup() {
			...
			var updated_csrf_token = $('#updated_csrf_token').val();
			if(updated_csrf_token == '') {
				updated_csrf_token = csrfData.formatted.csrf_token_name;
			}
			
            $.ajaxSetup({
                //data: csrfData.formatted
                data: {"csrf_token_name" : updated_csrf_token}
            });

            $(document).ajaxError(function(event, request, settings) {
                if (request.status === 419) {
                    alert_float('warning', 'Page expired, please reload the page again to perfom any action.')
                }
            });
...
$( document ).ajaxComplete(function() {
	...
	<input type="hidden" name="updated_csrf_token" id="updated_csrf_token" value="" />
<?php
}
...
function random_string($length = 10, $add_dashes = false, $available_sets = 'luds')
{
    $sets = array();
...
function general_validate($data)
{
    $data = trim($data);
...
function setDateFormat($date){
    return (!empty($date))?date('d-M-Y',strtotime($date)):$date;
}
...
function cryptoJsAesDecrypt($passphrase, $jsonString){
	...
	//echo $passphrase.'====>'.$jsonString;exit;
	try {
...
function random_salt_token() {
	#for php5
	//$_SESSION['random_salt'] = bin2hex(openssl_random_pseudo_bytes(32));
	
	#for php7
	$_SESSION['random_salt'] = bin2hex( random_bytes(32) );
	
	return $_SESSION['random_salt'];
}


// function getSmshelper($mob,$message,$templateId,$citizen){
//     if(!empty($citizen)){
//     $encoded_message = urlencode($message);
//     $url = "http://sms.technixindia.com/http-tokenkeyapi.php?authentic-key=37354249485043423130301646829696&senderid=PCBBIH&route=1&number=".$mob."&message=".$encoded_message."&templateid=".$templateId;
//     // echo $url;exit;
//     $ch = curl_init();
...
//     } else {
//        $status =  'fail to retrieve SMS status from the response';
//     }
...
function getSmshelper($mob, $message, $templateId, $citizen) {
    // Check if $citizen is not empty
    if (!empty($citizen)) {
                    $messages =  urlencode($message);
            $ch = curl_init();

            curl_setopt_array($ch, array(
            CURLOPT_URL => 'http://sms.technixindia.com/http-tokenkeyapi.php?authentic-key=37354249485043423130301646829696&senderid=PCBBIH&route=1&number='.$mob.'&message='.$messages.'&templateid='.$templateId,
        ...
        // $data = [
        //     'authentic-key' => '37354249485043423130301646829696',
        //     'senderid' => 'PCBBIH',
        //     'route' => '1',
        //     'number' => $mob,
        ...
        // // Set cURL options
        // curl_setopt($ch, CURLOPT_URL, "http://sms.technixindia.com/http-tokenkeyapi.php");
        // curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        // curl_setopt($ch, CURLOPT_TIMEOUT, 30);
        // curl_setopt($ch, CURLOPT_POST, true);
        //curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($data)); // Sending POST data
        // Build the URL for the SMS API
        // $url = "http://sms.technixindia.com/http-tokenkeyapi.php?authentic-key=37354249485043423130301646829696&senderid=PCBBIH&route=1&number=" . urlencode($mob) . "&message=" . $encoded_message . "&templateid=" . urlencode($templateId);
        // // Initialize cURL
        // $ch = curl_init();
        ...
        // curl_setopt($ch, CURLOPT_URL, $url);
        // curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        // curl_setopt($ch, CURLOPT_TIMEOUT, 30); // Optional: Timeout to prevent hanging
        ...
        if ($response_data && isset($response_data['Status'])) {
            return $response_data['Status']; // Return the status from the response
        } else {
            return 'Failed to retrieve SMS status from the response';
        }
    } else {
        return 'Citizen information is required';
    }
}
