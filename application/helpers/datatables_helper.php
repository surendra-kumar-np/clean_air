 ...
 * @return array
 */
// issue category table start
function data_tables_init_category($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [], $addOrderBy = '')
{
    $CI          = &get_instance();
            ...
            if (stripos($columnName, 'AVG(') !== false || stripos($columnName, 'SUM(') !== false) {
                ...
                if (($__post['columns'][$i]) && $__post['columns'][$i]['searchable'] == 'true') {
                    ...
                    if ($useMatchForCustomFieldsTableSearch === 'true' && startsWith($columnName, 'ctable_')) {
                        ...
                        $sWhere .= 'convert(' . $columnName . ' USING utf8)' . " LIKE '%" . $CI->db->escape_str($search_value) . "%' OR ";
                    }
                }
            }
        }
        ...
        if (count($additionalSelect) > 0) {
            foreach ($additionalSelect as $searchAdditionalField) {
                ...
                if (stripos($columnName, 'AVG(') !== false || stripos($columnName, 'SUM(') !== false) {
                    ...
                    // Use index
                    $sWhere .= 'convert(' . $searchAdditionalField . ' USING utf8)' . " LIKE '%" . $CI->db->escape_str($search_value) . "%' OR ";
                }
            }
        }
    ...
    $sQuery = '
    SELECT COUNT( DISTINCT ' . $sTable . '.' . $temp_coulumn . ")
    FROM $sTable " . $join . ' ' . $where;
    ...
    $_query = $CI->db->query($sQuery)->result_array();
    $iTotal = $_query[0]['COUNT( DISTINCT ' . $sTable . '.' . $temp_coulumn . ')'];
    /*
     * Output
     */
    $output = [
...
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [])
{
    ...
    if ((isset($__post['search'])) && $__post['search']['value'] != '') {
        ...
        if (count($additionalSelect) > 0) {
            foreach ($additionalSelect as $searchAdditionalField) {
                // pre([$searchAdditionalField, strpos($searchAdditionalField, "GROUP_CONCAT")]);
                if (strpos($searchAdditionalField, "GROUP_CONCAT") !== false) {
                    $exception_str = 'convert(' . strtok($searchAdditionalField, ")") . ') USING utf8)' . " LIKE '%" . $CI->db->escape_str($search_value) . "%'";
                    // pre(strtok($searchAdditionalField, ")"));
                    // pre($searchAdditionalField);
                    continue;
                }
                if (strpos($searchAdditionalField, ' as ') !== false) {
                    $searchAdditionalField = strbefore($searchAdditionalField, ' as');
    ...
    if ($exception_str && $exception_str != "") {
        // $sGroupBy = $sGroupBy . " " . "HAVING " . $exception_str;
    }
 ...
 * @return array
 */
function get_issue_field_data($pid, $column){
    $CI = &get_instance();
    $CI->db->select($column);
    $CI->db->where('id', $pid);
    ...
    if($rows){
        if($column == 'region_id'){
            $str = '';
            $data = json_decode($rows[0][$column], true);
            if(is_array($data)){
                $str = implode(",",$data);
            }
            return $str;
...
function get_milestone_of_issue($id, $key)
{
    $CI = &get_instance();
    if (get_roleid(get_staff_user_id()) == 2) {
        $parentissueid = 0;
        $areaid = 0;
    } else {
        $areaid = get_staff_area_id(get_staff_user_id());
    }
    $CI->db->select($key);
    $CI->db->where('issue_id', $id);
    $CI->db->where('area_id', $areaid);
    $CI->db->where('is_active', 1);
    $rows = $CI->db->get(db_prefix() . 'issue_milestones')->result_array();
    $count = 0;
    $milestone = array();
    foreach ($rows as $row) {
        $milestone[$count] = $row[$key];
        $count++;
    }
    return implode(',', $milestone);
}
function calcuatetotaldurationofcategory($id, $key)
{
    $CI = &get_instance();
    $CI->db->select($key);
    $CI->db->where('issue_id', $id);
...
function check($id)
{
    $CI = &get_instance();
    $CI->db->where('issue_id', $id);
    $CI->db->where('staff_id', $CI->session->userdata('staff_user_id'));
    $rows = $CI->db->get(db_prefix() . 'issue_region')->result_array();
    if (empty($rows)) {
        return true;
    } else {
        return false;
